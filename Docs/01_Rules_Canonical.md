...
高球传中明确适用本节：进攻侧计入本次实际参与的持球与跑位球员，防守侧计入盯人与实际存在的协防球员；按该权威参与集合取体力总和，不只比较持球与盯人。缺少协防不构造球员，也不添加其体力。持球/跑位、盯人/协防必须是不同球员；实际参与门将不能兼任盯人或协防，不能重复计数。门将参与的总值平局仍优先适用门将防守获胜规则，不进入体力比较；快速压制及非平局总值比较的优先级不变。

8.8F Closeout 修正了高球传中组装只传入持球/盯人体力的实现偏差。此修正不扩展到低球传中；低球参与体力组装与本节一般规则的后续一致性核查另行处理，不据此改写多人求和的规则意图。
