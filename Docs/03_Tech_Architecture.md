...
- Network writes admit exactly Full D12, DeployOrdinary, DeployGoalkeeper and FinishDeployment. The common boundary validates participant, match, positive correlation, closed kind/payload shape, bounded increasing RequestId and current attack sequence. It does not assume the acting player is the attacker.
...
- The owner Controller uses the same generated RPC for listen host and remote. Client correlation/pending/ACK handling is shared across all four kinds, separate from intent-specific DEV display. No network gameplay resolver, generic workflow engine or second command registry is introduced.
