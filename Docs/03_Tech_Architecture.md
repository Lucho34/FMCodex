...
- SubmitSkill is the next narrow unnetworked choice on the Skill-wait route. DeclineHelper/DeclineRunner remain independent unnetworked choices; ResolveNoLegalHelper remains server-internal and applies only when no legal Helper exists. Voluntary DeclineHelper requires at least one legal Helper.
