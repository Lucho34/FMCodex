...
- MarkerSelection now offers the complete safe Marker choices through the narrow transport below. DeclineMarker, DeclineRunner, DeclineHelper, Skill and rolls remain outside the network allowlist.
...
- Only the acting viewer receives RunnerOptions. SelectedRunner copies safe SelectedRunnerCardId and attacker roster display data. Disclosed ordinary selections are public; withholding the route clears participants before network projection. No raw State, future route, Formula or RNG data is added; Skill candidates use the narrow safe projection below.
- Runner writer freezes Runner and enters AwaitingHelper. With a legal Helper, Coordinator stops there expecting the defender, with SkillId/ActionType empty, bSkillSelectionDeferred true, no selected action and no resolution session. SubmitHelper uses the narrow transport below; DeclineHelper remains unnetworked.
...
## SubmitHelper transport boundary

- FMatchPlayAuthoritativeSubmitHelperRequest contains ExpectedAttackSequence (observed common freshness token, server validated), RequestingSide (ParticipantRegistry-derived) and HelperCardId (the sole player choice). The adapter constructs this exact request. Local typed SubmitHelper and Network use the same FMatchPlayEntryDeploymentPlayerIntentPort, Session.SubmitHelper and one Coordinator pass after domain success.
- CoreRules owns defending-side authority, Resolution/AwaitingHelper, frozen Carrier/Marker/Runner, unique own deployment and snapshot, non-GK eligibility, Marker exclusion and the shared physical half of Runner. No separate Available/Used/Ejected, position or tactical-match condition is invented in transport.
- BuildForViewer -> SelectHelper -> SelectionOptions/RelatedCardId is the complete legal source. CopyCompleteSelectionOptions serves all four typed participant projections; no raw State or independent Network legality calculation. The defending deck has at most 18 candidates after excluding its one GK and frozen Marker. The current five-slot Runner half has at most four, attained when Carrier/Marker occupy the other half.
- HelperOptions, bHelperOptionsUnavailable and SelectedHelper follow existing safe identity/name disclosure. Preserve order and every candidate; overflow above 18, duplicate/invalid identity or wrong source Side fails the entire projection diagnostically. Nonacting viewers receive no actionable list. SelectedHelper follows safe bHasHelper/SelectedHelperCardId and defender roster display data; withholding or canonical attack closure clears it.
- Helper finalization reaches AwaitingSkill on participant-first attacks. With a legal Skill, Coordinator stops there with SkillId/ActionType empty, bSkillSelectionDeferred true and no selected action/resolution session. Otherwise ResolveNoLegalSkill may complete the attack and expose the next Full D12 wait. Neither path executes a player choice; transport must not retain obsolete participants or claim a Skill wait after closure.
- SubmitSkill on the Skill-wait route uses the narrow transport below. DeclineHelper/DeclineRunner remain independent unnetworked choices; ResolveNoLegalHelper remains server-internal and applies only when no legal Helper exists. Voluntary DeclineHelper requires at least one legal Helper.

## SubmitSkill transport and server-pinned rules

- The ninth admitted intent is SubmitSkill. The canonical request is exactly ExpectedAttackSequence, RequestingSide and SkillId. The wire adds only bounded SkillId; common validated sequence and ParticipantRegistry Side reconstruct the request. Both owning controllers call generated ServerSubmitPlayerIntent; Local typed dispatch and Network use FMatchPlayEntryDeploymentPlayerIntentPort, production Session.SubmitSkill(Request), and one Coordinator pass after domain success.
- Session copies AuthoritativeSkillRuleSet at construction. Skill identity resolves only against that private pinned copy. Neither client payload nor shared port accepts a rule set, ActionType, TP range, participant requirement, modifier, Formula, threshold, branch, roll or state transition. The automation-only fixture overload remains outside the production path.
- CoreRules validates current attack/sequence, Resolution/AwaitingSkill, current attacker, frozen Carrier/Marker context, unique deployments and authoritative Carrier snapshot. SkillId must be owned by that Carrier, resolve in the pinned rules and satisfy current TP plus canonical participant requirements. PassControl requires midfield Runner; Cross requires attack-position Runner; ThroughBall requires attacker-forward physical area. Helper is optional. Deferred selection is the current participant-first path, not an invented universal gate for legacy canonical paths.
- BuildForViewer supplies the complete legal SelectSkill list. Each source Id is the exact runtime SkillId; RelatedCardId is the Carrier and must never substitute for Skill identity. CopyCompleteSelectionOptions now takes a typed identity/copy callback, shared with the four participant projections, with no legality calculation. MaxSkillOptions=3 covers the canonical snapshot bound; current catalog TP overlap is at most two, attained by a valid current fixture. Overflow, duplicates, invalid encoding or wrong source Side fail the entire list.
- SelectedSkillId is projected from canonical preparation/selected action into the existing safe InteractionView and cleared with undisclosed ordinary attack facts. Network SelectedSkill copies that safe identity and the existing PresentedActionType Chinese label. Only the acting viewer receives Skill controls. No rule object, raw State, future route, Formula or RNG state is replicated.
- Participant-first LongShot, CutInsideShot and Cross freeze Skill/type and stop at AwaitingBranchIntent, expecting SubmitBranchIntent. LongShot/CutInside clear unused prepared Runner/Helper; Cross preserves them. PassControl/ThroughBall freeze SelectedAction and stop at ReadyForResolution, expecting ResolvePassControlInitialRouteRoll / ResolveThroughBallInitialRouteRoll respectively. All five remain in Resolution, clear deferred Skill, create no resolution session or route outcome and require the attacker next. Coordinator performs zero internal steps for these accepted choices.
- ResolveNoLegalSkill remains server-internal and closes the attack only when no legal Skill exists. DeclineSkill is a distinct canonical attacker choice when legal Skills exist; it also closes the attack and remains unnetworked. Both preserve the next Full D12 wait. DeclineRunner/DeclineHelper remain separate debt.

## Ordinary tactical branch transport family

- SubmitBranchIntent is the tenth closed network intent and one shared canonical family for LongShot, CutInsideShot and Cross. FMatchPlayAuthoritativeSubmitBranchIntentRequest contains AttackSequence (not ExpectedAttackSequence), RequestingSide and EMatchPlayElectiveBranchIntent Intent. The adapter maps the common ExpectedAttackSequence to AttackSequence, derives Side from ParticipantRegistry and copies only Intent.
- Both owning controllers call generated ServerSubmitPlayerIntent. Local typed branch and Network Runtime use FMatchPlayEntryDeploymentPlayerIntentPort -> Session.SubmitBranchIntent -> canonical writer -> one Coordinator pass after success. Neither LocalPlay nor authoritative dispatch depends on a local viewport, network correlation fields or RPC.
- The frozen server ActionPreparation.SkillId/ActionType supplies branch context. Branch selection does not accept or re-resolve a client Skill/rule set. Canonical global context validates attacker, Resolution/AwaitingBranchIntent, canonical selection state, unique deployed Carrier/Marker and authoritative snapshots; Cross also validates its distinct attack-position Runner and optional Helper.
- The canonical sets are DirectShot/DeadCorner for both LongShot and CutInsideShot, and CrossHigh/CrossLow for Cross. Cross values under shot Skills, and shot values under Cross, are rejected by canonical legality. DirectShot/DeadCorner legitimately overlap between LongShot and CutInside; they are interpreted using the frozen server Skill and do not constitute cross-Skill forgery.
- BranchOptions copies the complete BuildForViewer.BranchIntentOptions in order. Each family has two options, so MaxBranchOptions=2. Duplicate, invalid enum or overflow fails the entire list diagnostically. Only the acting viewer receives controls. Labels reuse MatchScreenLabel: 直接射门、射向死角、高球传中、低球传中. SelectedBranch copies safe ElectiveBranchIntent and is public after acceptance when the ordinary attack is disclosed; withholding clears it.
- The writer freezes SelectedAction.ElectiveBranchIntent and enters ReadyForResolution. Cross stops there with no resolution session or actual route. LongShot/CutInside then canonically execute BeginResolutionSession and ResolveIntentDeterminedRoute: the session reaches RouteResolved without RNG, retaining ReadyForResolution selection state. They stop at their direct-attack or paired dead-corner player roll. These two internal steps are not client decisions.
- Network projects only branch identity/label and the high-level next wait. No actual-route DTO, dice, Formula, result or next-stage RPC is added. Accepted branch coordinates/publishes once; rejection coordinates/publishes zero times. Existing pinned Skill rules, secure randomness and generic ledger/pending remain unchanged.

## Initial-route roll transport family

- Cross, PassControl and ThroughBall retain separate canonical PlayerIntent identities: ResolveCrossInitialRouteRoll, ResolvePassControlInitialRouteRoll and ResolveThroughBallInitialRouteRoll. Each corresponding FMatchPlayAuthoritativeResolve<Family>InitialRouteRollRequest contains int64 AttackSequence and EInitialTurnOrderPlayer RequestingSide only. The network adapter maps ExpectedAttackSequence and derives Side from ParticipantRegistry; it never accepts a die, branch, Skill, provider or result.
- Local typed dispatch preserves its existing ExecuteProviderCall DEV invocation decorator around FMatchPlayEntryDeploymentPlayerIntentPort. Network Runtime uses the same port. Each case validates the exact canonical variant, calls Session's existing initial-route orchestrator, then invokes Coordinator once after success. Legacy Local result facades remain available; no Local dependency on RPC/correlation is introduced.
- Session validates current attack, positive matching sequence, current attacker, frozen family and ReadyForResolution/no-session or AwaitingRoute/unresolved-empty state before RNG. Cross requires the already selected canonical High/Low intent; PassControl/ThroughBall use no elective intent. Canonical binding and route writer retain participant/Skill snapshot validation. Orchestration failure does not adopt its candidate session.
- The private initial-route counting decorator delegates production draws to the existing PlatformCrypto-backed Network random provider. An accepted command commits one initial D6; invalid/replayed/rerolled requests do not call that provider. Provider failure does not substitute RNG or automatically retry. A non-Shipping server-only automation override can supply a test D6 through that same provider interface.
- Successful route plus stable Coordinator completion records the exact attack's semantic route disclosure. BuildForViewer receives only the initial-entry and route reveal permissions for that attack, with contest count zero and terminal reveal false. It remains the sole source for the network action/result projection; no FullyDisclosed path or independent D6-to-route interpreter is added.
- Every initial-route outcome stops in Resolution / ReadyForResolution with a RouteResolved session, one initial route record, no post-route roll records, no terminal and the attacker as next actor. Coordinator executes zero internal steps for these commands: beginning the session and drawing the route belong inside the single canonical Session orchestration, not separate Coordinator actions.
- The next waits are Cross High/Low Attack, PassControl Attack, or ThroughBall Feet / BehindDefense P1 / AntiOffside Attack. These player rolls and their Formula/result transport remain unimplemented in NetworkPlay.

## Cross contest transport and automatic terminal persistence

- ResolveCrossHighAttackRoll, ResolveCrossHighDefenseRoll, ResolveCrossLowAttackRoll and ResolveCrossLowDefenseRoll are canonical PlayerIntents. Their four exact FMatchPlayAuthoritativeResolveCross…RollRequest types (listed in Data Schema) contain only int64 AttackSequence and EInitialTurnOrderPlayer RequestingSide. The adapter maps common freshness and registry Side into the existing request; no gameplay choice is added.
- Shared FMatchPlayEntryDeploymentPlayerIntentPort calls the four existing AuthoritativeSession methods. Local typed dispatch retains its four ExecuteProviderCall DEV invocation decorators around that same port. Legacy Local result facades remain compatible; LocalPlay has no Network dependency.
- FMatchPlayCurrentAttackResolveCrossPostRoutePlanOrchestrator validates the current attack, canonical session, actual Cross High/Low route, pending PrimaryAttack/PrimaryDefense purpose, and the corresponding attacking/defending Side before invoking IMatchPlayPostRouteRollProvider. Selected CrossHigh/CrossLow remains frozen intent, not contest-route authority. The initial route may flip it.
- Every accepted step commits exactly one D6. The private counting decorator delegates production to the existing PlatformCrypto-backed Network provider. The server-only non-Shipping automation seam supplies explicit test PrimaryAttack/PrimaryDefense values through that interface; it is not a second production RNG implementation.
- Shared dispatch invokes Coordinator once after success, never after rejection. Attack leaves PrimaryBranch with one roll and zero internal steps, awaiting the defender. Defense leaves two rolls and permits the existing single ApplyCrossTerminalResolution internal step. That step regenerates the canonical finishing Formula from persisted records with zero RNG, persists Goal/NoGoal and any score/GoalHistory effect, and stops at TerminalPendingAdvance.
- Both stages retain Resolution / ReadyForResolution and RouteResolved. Terminal persistence preserves roles, cards, used opportunity counts, sequence and current attacker until the genuine AdvanceAfterTerminal PlayerIntent. Network has no terminal-apply RPC or advance RPC.
- Earlier Cross interaction text describing a separate terminal confirmation or immediate handoff is not the shared Coordinator contract. The later terminal-persistence architecture and current Session/Coordinator control this boundary: automatic persistence followed by explicit advance.
- Network grants only the accepted Cross contest prefix for the exact attack to BuildForViewer. The narrow CrossContest fact contains AttackD6, DefenseD6 and bFormulaResolved; the action enum is derived from safe interaction plus disclosed typed ActualBranch. Formula totals, winner, Goal/scorer and full Session objects are not copied. Terminal outcome permission remains false, so current-attack score/history redaction remains in force even after server persistence.

## Cross network terminal disclosure and explicit advance

- Authority terminal persistence and viewer permission are separate. `ApplyCrossTerminalResolution` persists the canonical Goal/NoGoal, scorer, score and GoalHistory while CurrentAttack remains TerminalPendingAdvance. This alone does not authorize public disclosure.
- Network Runtime holds an exact-attack terminal permission, independent of its entry/route/contest prefix permissions. A successful Cross Defense intent followed by the existing Coordinator's stable terminal completion grants that permission. BuildClientView requires the same terminal attack and both disclosed contest rolls, then calls `FFMCodexLocalMatchInteractionViewBuilder::BuildForViewer`. Its existing `RedactTerminalOutcome` removes the current GoalHistory record, score increment and decisive Goal/Miss facts while permission is absent.
- The bounded Network DTO copies only that safe view: the unique resolved Cross.High.Outcome/Cross.Low.Outcome decision supplies Goal or NoGoal, and the same attack's safe GoalHistory supplies the scorer. Goal, scorer, scoreboard and history therefore enter the same owner View revision. NoGoal carries no scorer or current goal entry. There is no Network Formula comparison, Runner-based scorer inference or raw authoritative history shortcut.
- Network DEV publishes a freshly completed stable terminal immediately. Local production retains its existing Reel, narrative and readable-hold timing in the Screen; `BuildDisplayedHeader` retains already displayed score labels until narrative disclosure. This does not add a second score authority. Future Network player-facing presentation must preserve the safe disclosure boundary when adapting those surfaces.
- Local typed Advance and Network use `FMatchPlayEntryDeploymentPlayerIntentPort`, `Session.AdvanceAfterTerminal` and one Coordinator pass after success. Advance consumes all deployed non-GK ordinary cards, preserves persistent GK usage, consumes exactly one attacking opportunity, derives the next attacker/MatchEnd, performs canonical Recovery only if non-final, and atomically clears CurrentAttack. Provider failure cannot adopt partial card/opportunity/score changes.
- Non-final Advance stops at the next side's Full D12 intent with no CurrentAttack object and no automatic roll. The safe view offers the next sequence; the existing Full D12 request creates that attack. Final Advance records MatchResult, clears Recovery, leaves no CurrentAttack and offers no next player action. A displayed sequence correlation value after full time does not indicate an extra attack.
- Recovery instrumentation is a private counting decorator around the existing secure provider, not a replacement sampler. Successful Advance clears all attack-local Network disclosure permissions before the next publication. Persistent public score/history remain; terminal/role/Skill/branch/roll facts clear through the canonical safe projection.

## Shared player-facing Local and Network adapters

UFMCodexLocalMatchScreenWidget remains the single match screen. Its existing header, racks, pitch cards, interaction dock, tactical detail, central resolution surfaces, Reel, Formula/Narrative and Full-Time widgets serve both modes. The historical Local class name does not imply Local authority in the widgets.

- Local read: Local Host/ViewPort → existing InteractionView/ResolutionFeedback → UMG presentation builder → screen. Local write: IFMCodexMatchScreenBackend → Local Controller's existing synchronous methods. Start, DEV tools, short match and unnetworked Local tactics stay Local; no match/request identifiers or RPC are introduced there.
- Network read: authoritative Session → BuildForViewer with exact-attack disclosure permissions → FFMCodexNetworkMatchPresentationAdapter::Project → the same owner-only snapshot/revision → Read → shared screen. The projection carries only the audited ordinary Cross display surfaces and accepted roll events. It never serializes State, Session, the internal InteractionView or generic ResolutionFeedback.
- Network write: shared typed screen gesture → FFMCodexNetworkMatchScreenActions → existing intent client correlation → generated ServerSubmitPlayerIntent → connection validation → shared authoritative port. Listen Host and Remote use this identical generated RPC entry. The interface distinguishes Local completion, Network queueing and rejection.
- OnRep, server owner publication and ACK refresh the latest presentation. UI construction also reads that retained snapshot. Pending gates input, not the presentation timer; ACK-first/View-first require both correlated halves, and rejection cancels an unresolved visual wait without changing gameplay facts.
- Presentation identity combines the match scope (Controller resets the screen for a new MatchInstanceId), attack sequence, roll kind, contest identity, roll sequence and owner. Existing settled-event guards prevent repeated View/ACK updates from replaying a result. New attack sequences may use the same roll purpose again.
- The Network adapter supplies at most route/attack/defense accepted roll events to the same Reel. Animation is cosmetic and settles to the disclosed server value. Formula/Narrative are formatted on the server from safe facts; the client does not resolve arithmetic or infer a winner.
- The existing displayed-header gate retains the score already painted before Defense until the local result/Narrative reveal permits the new safe score. This is a presentation suspense gate; accepted safe results can already be present on the client.
- Advance remains an explicit owner intent. Its resulting snapshot clears old attack presentation, hands Full D12 to the next actor, or opens the same Full-Time widget. Neither viewport existence nor animation completion is required by authority.

The shared action seam is bounded to real screen gestures, not a general workflow engine. Local-only set-piece conveniences remain attached to the Local Controller until those families have an audited Network contract.

The narrow action projection keeps its accepted-roll-only safe view. The display projection uses a second BuildForViewer call on the same immutable snapshot and revision, opting into unresolved Cross Formula placeholders. A withheld accepted roll still removes all derived Formula/decision data. Logout UI refresh also checks that a non-null Player is an actual LocalPlayer, and Dedicated net mode never constructs this UI.

If replication coalesces several accepted Cross events, an already-observing screen drains the remaining disclosed prefix through the existing route/attack/defense timeline. A late-created screen displays the latest state without replaying unobserved history. Local consumers do not populate this bounded event list and retain their original path.

The shared interaction dock has separate status and control visibility. In a viewer-relative mode, central ownership suppresses only the dock controls; the existing actor/action text remains available to both viewers after the same reveal gate as the CTA. Network Read derives this presentation from existing ExpectedActingSide, ViewerSide and typed interaction category. No replicated prompt field or new authority is required. Local hot-seat retains its existing actor labels and dock behavior.

## Ordinary sequential contest transport and shared presentation

Cross, all three PassControl routes and ThroughBall Feet reuse the same owning RPC, connection ledger, typed HostPort, serialized Session and Coordinator. PassControl Defense already includes atomic terminal persistence in its Session command; Feet Defense permits the existing ApplyThroughBallTerminalResolution Coordinator step. Preserve those authoritative continuations: both stop at TerminalPendingAdvance, and neither requires a player Formula/terminal-application command. Only explicit AdvanceAfterTerminal consumes the opportunity and performs the existing non-final Recovery / next-actor transition or final MatchEnded transition.

The existing contest and terminal projection is shared as Contest / Terminal, with one exact-attack disclosed prefix and terminal permission. The display-only unresolved-descriptor permission covers Cross, PassControl and actual ThroughBall Feet. Withholding any accepted D6 still removes its derived Formula/decision data. Network copies the existing PassControl branch surface and ThroughBall surface into the bounded safe presentation; the same LocalMatchScreenWidget renders and paces them. Initial-route event kinds must match the existing Cross, PassControlInitialRoute or ThroughBallInitialRoute identities exactly. Generic ACK/View pending, event deduplication, mirrored action/wait prompts and displayed-score reveal remain shared. ResultHold uses elapsed world game time; transport integration does not tune its constants.

PassControl is a complete player-facing ordinary capability. ThroughBall remains disabled in general Network tactic selection because BehindDefense, AntiOffside and OneOnOne continuations are incomplete. A non-Shipping server Feet milestone may enable ThroughBall for its exact prepared attack only, using the existing initial-route RNG seam; the capability grants no client control over route, dice or outcome.

## ThroughBall conditional Network continuation (Stage 7.17)

The current player-facing Network capability includes all canonical ThroughBall routes: Feet, BehindDefense, AntiOffside, and the shared OneOnOne Direct/Chip continuation. This supersedes the temporary Feet-only capability described above. LocalPlay remains independent of Network setup and both use the original LocalMatchScreenWidget and authoritative gameplay implementation.

Six additional empty roll intents and the canonical OneOnOne choice traverse the existing generated owning RPC, connection registry, common envelope validation, typed HostPort and Session. No Formula/application/Recovery client action is added. BehindDefense's canonical completed single-roll gate goes directly to ApplyThroughBallTerminalResolution; the two-sided Formula is only available after the conditional Defense. Coordinator must not call that Formula for the OutOfPlay gate. Terminal regeneration validates the result, with no new RNG or duplicated D6 mapping.

Accepted roll publication supports the primary prefix followed by distinct OneOnOne purposes. Security-hidden accepted conditional rolls also hide dependent choice/actor/pending descriptors, formulas, decisions and current terminal score/history. A skipped authoritative BehindDefense Formula can preserve its public gate decision when its unused Defense descriptor is removed and all accepted rolls are disclosed. Display-only unresolved operands remain opt-in. Normal Network publication immediately discloses accepted public rolls; shared Reel/ResultHold controls when the result and score become visible.

The existing central ThroughBall surface owns route, conditional rolls, OneOnOne choice, Formula/outcome, Narrative and terminal CTA. The lower dock mirrors the typed actor/action, without duplicate controls. Advance, Recovery, next Full D12 and Full-Time use the existing lifecycle. No shared Screen timer or gameplay-rule implementation changes are needed.

## Specialized ordinary shots over the shared Network boundary (Stage 7.18)

LongShot and CutInsideShot retain their canonical DirectShot and DeadCorner implementations. Six exact PlayerIntents pass through the existing Screen → Network action adapter → owning-controller generated Server RPC → common connection/envelope validation → typed shared HostPort → AuthoritativeSession → ServerCoordinator path. Host and Remote use the same path. No Formula, result, terminal persistence, Recovery or MatchEnd RPC is added. LocalPlay keeps its original synchronous adapter and the same authoritative implementation.

DirectShot requests one attacker D6. An accepted 1–2 is ImmediateMiss; 3–6 requires the defender's separate D6 and canonical Finishing Formula. LongShot uses Carrier LongShot versus Marker Tackling +2, with an active goalkeeper's Positioning ×0.5; CutInside uses the average of Carrier Shooting and Dribbling versus Marker Tackling +2, with active GK Handling ×0.5. Existing Tactical Player and tie policies remain authoritative. DeadCorner requests PairedAttackA and PairedAttackB atomically from the post-route provider; sum ≥11 is Goal. It has no attribute/GK contest or Defense action.

LongShot terminal persistence is the existing Coordinator ApplyShotTerminalResolution step. CutInside's roll transaction already adopts its canonical shot terminal atomically. Both stop at TerminalPendingAdvance and reuse explicit AdvanceAfterTerminal. Scorer is read from persisted GoalHistory; Network does not derive it from tactical roles. Shared BuildForViewer projection, exact-attack disclosure permission and the original Screen own public and visible reveal boundaries respectively.

## Voluntary optional choices over Network (Stage 7.19)

DeclineRunner, DeclineHelper and DeclineSkill are three genuine typed PlayerIntents. They follow the same owning generated Server RPC for Host and Remote, common connection identity/envelope ledger, exact shared HostPort dispatch, serialized Session and one Coordinator pass. LocalPlay keeps its original backend and gameplay implementation. No new rule, Formula, RNG, terminal, Recovery or opportunity-consumption logic is introduced.

The exact request types are FMatchPlayAuthoritativeDeclineRunnerRequest, FMatchPlayAuthoritativeDeclineHelperRequest and FMatchPlayAuthoritativeDeclineSkillRequest. Each contains only ExpectedAttackSequence and RequestingSide; the latter is connection-derived. Each requires at least one canonical legal option at its corresponding Resolution selection wait. ResolveNoLegalRunner, ResolveNoLegalHelper and ResolveNoLegalSkill remain Coordinator-owned ServerInternalActions, available only for canonical absence of options; they are never RPC capabilities.

Runner decline is owned by the attacker: formally remove Runner/Helper, clear deferred Skill/action, and query Skill availability at AwaitingSkill. Helper decline is owned by the defender: keep Runner, formally omit Helper, then reach AwaitingSkill in participant-first flow. Skill decline is owned by the attacker and directly completes the attack with unchanged score/history, consumes the opportunity, clears old roles/deployment and hands off to Full D12 or MatchEnded. It does not persist TerminalPendingAdvance and requires neither an Advance command nor a fabricated result Narrative. These declines use no gameplay RNG. If participant absence leaves no legal Skill, Coordinator may perform the existing internal no-Skill completion instead of publishing a dead wait.

## Marker early completion and shared transition feedback (Stage 7.20)

DeclineMarker now uses the same owning generated RPC, common ledger, connection-derived identity and exact typed EntryDeploymentPlayerIntentPort dispatch as the other optional choices. The port calls the existing serialized Session.DeclineMarker and one Coordinator pass. ResolveNoLegalMarker remains internal. Neither path gains new rules, RNG, Formula, scorer or lifecycle code.

Both canonical Marker-absence paths call CompleteMarkerGoal and atomically persist a team goal, consume the opportunity/deployed non-GK cards and clear CurrentAttack. They advance directly to the next attacker's Full D12 wait or MatchEnded. CD-069's pre-resolution exception applies: no TerminalPendingAdvance, extra Advance or Recovery is invented. Both sides lacking a legal Carrier resolve through the earlier no-Carrier NoGoal path; Marker requires a canonical frozen Carrier. Later Runner/Skill optionality remains phase-ordered, without a new cross-stage preflight.

A shared BuildCompletedSystemGoal feedback builder formats only an already-public GoalHistory entry with bSystemAward and no scorer for the immediately completed attack. Local's synchronous adapter and Network's safe DTO reader reuse it in the existing nonblocking resolution surface. Final matches use Full-Time. No new persistent outcome/cause is guessed; the generic label is 规则判定进球.

Recovery presentation audit found the existing Local BuildRecovery notification was missing from the Network read adapter. Network now supplies only the existing safe Recovery entries to that same formatter and shared surface. A local two-second world timer keyed by match/completed attack prevents repeated ACK/View refresh from restarting it; entering a new attack clears it. No shared Screen lifecycle, replicated timer, raw State or production gameplay dependency is added.

## Set-piece entry and selection transport (Stage 7.21)

RequestSetPieceTypeRoll, SubmitSetPieceCarrier, SubmitShortFreeKickMethod, SubmitLongFreeKickMethod and SubmitPenaltyMethod are existing canonical PlayerIntents. Host and Remote now submit them through the same generated owning-controller RPC, common connection-derived Side/envelope ledger, exact typed shared HostPort, serialized Session and one Coordinator pass. No CoreRules, Formula, Session or Coordinator gameplay implementation is duplicated or changed. LocalPlay keeps its synchronous backend and the original hand draft → confirm → method interaction.

Full D12 9–12 enters SetPiece/AwaitingTypeRoll. Type consumes one D6 from the existing private entry provider; 1–2 selects Corner, 3–4 Long Free Kick, 5 Near Free Kick and 6 Penalty. Type-dependent values enter the safe View only after exact-attack disclosure permission. Accepted public type events reuse SetPiece.Type, sequence index 0, attacker and attack sequence in the shared Reel/ResultHold dedupe path. Replication is immediate; visible type/taker/method presentation waits for the existing elapsed-time reveal.

The non-Corner taker set is the canonical complete available own non-GK set, bounded by the admitted 20-card roster and unique GK (19 maximum). A bounded safe presentation value carries IDs, labels, stages and distinct typed method options; it carries no participant snapshot or future resolution facts. Near Angled eligibility is projected from the existing authoritative frozen-taker rule. UMG does not reconstruct it from card attributes. Overflow, duplicate or invalid options fail closed.

The actual specialized state remains CurrentAttack.Phase=RoutePending, ordinary SelectionStage=None. Choosing a method stops at its existing decisive PlayerIntent; no decisive roll, Formula or goal capability is added. Corner stops at AwaitingAttackerNominations with no nomination capability or sealed data. The already-automatic no-legal-taker path remains Coordinator-owned, persists canonical NoGoal and uses the existing terminal/Advance contract with narrowly scoped safe disclosure. It does not acquire a client ResolveNoLegal command.

## Near Free Kick resolution and shared presentation (Stage 7.22)

The complete Near / ShortFreeKick family adds only three existing PlayerIntents: ResolveShortFreeKickDirectAttackRoll, ResolveShortFreeKickDirectDefenseRoll and ResolveShortFreeKickAngledRoll. Each uses FMatchPlayShortFreeKickRollRequest (connection-derived RequestingSide and the common AttackSequence) through the same generated RPC, common validation/ledger, typed HostPort, serialized Session and one Coordinator pass. LocalPlay retains its synchronous backend. No CoreRules, Session, Coordinator, Formula, scorer or Recovery implementation is copied or changed.

Direct always accepts one attacker D6 and waits for the defender, including attacker values 1–2. The separate defender D6 resolves the existing Finishing Formula: max(frozen taker Shooting, Passing) + attack D6 versus unique defending GK Handling +1 + defense D6. Both TacticalPlayerModifier values are zero. Existing 6 versus 1/2 fast suppression and GK-wins-tie apply. Angled requires frozen Shooting + Passing >=8 for method selection and resolves two attacker D6 atomically, A then B; sum >=9 is Goal. It has no arithmetic Formula, defender roll or participant modifier.

Both final roll transactions already persist their Goal/NoGoal, scorer and terminal through the canonical completion writer. Coordinator stops at TerminalPendingAdvance with no new internal resolution command. AdvanceAfterTerminal consumes the opportunity/taker and uses existing Recovery or MatchEnded. Recovery can return the just-consumed taker to Available; final state performs no Recovery and offers no new D12.

Exact-attack Near disclosure records the accepted prefix separately from ordinary route rolls. Hidden defense/pair values, Formula winner, terminal affordance, current score and GoalHistory stay out of the safe projection. Once legally public, accepted values and terminal facts replicate immediately. The original shared Screen applies Reel, actual elapsed ResultHold, Formula/Narrative and displayed-score gates. Near uses its existing local event identities; the common Network event queue follows accepted array order across contests whose local index may both be zero. A coalesced Direct terminal cannot display defense totals or terminal Narrative while its attack event is revealing.

Near is enabled generally, including common type entry and legal taker/method selection. Long/Penalty/Corner retain their explicit incomplete continuation gates. The optional Near fixture supplies only host-side non-Shipping provider dice; it does not choose a taker/method or force a route/result. USER PIE remains required for final visible acceptance.

## Long Free Kick resolution and shared free-kick projection (Stage 7.23)

Complete LongFreeKick adds only the existing PlayerIntents ResolveLongFreeKickDirectAttackRoll, ResolveLongFreeKickDirectDefenseRoll and ResolveLongFreeKickPowerRoll. All use FMatchPlayLongFreeKickRollRequest { RequestingSide, AttackSequence } through generated owning RPC, common connection identity/closed envelope/ledger, the typed shared HostPort and serialized Session. No CoreRules, Formula, Session, Coordinator, Recovery or scoring implementation is changed.

Direct attacker D6 1 or 2 persists early NoGoal immediately, without a defense roll or full comparison. Values 3–6 persist the attack prefix and wait for the defender's D6. The exact FormulaType is Finishing: frozen taker LongShot + attack D6 versus defending GK Positioning +2 + defense D6, zero ordinary TacticalPlayerModifier, existing FastSuppression and GK-wins-tie. Power is outcome-only: one request draws LongFreeKickPowerA then LongFreeKickPowerB atomically; sum >=11 is Goal, otherwise NoGoal. Power has no attribute eligibility, defender roll or arithmetic Formula.

All decisive/early terminal requests already persist outcome, scorer and GoalHistory inside the existing resolution transaction. Coordinator stops at TerminalPendingAdvance without a new internal continuation. Existing AdvanceAfterTerminal consumes taker/opportunity and performs Recovery or MatchEnded; a recovered taker can immediately become Available again. Final Advance has no Recovery or next D12.

Near and Long now share bounded free-kick accepted-roll projection and exact-attack disclosure bookkeeping. The persisted Long early NoGoal shape requires one disclosed roll; comparisons and pairs require two. This requirement is derived before redacting any later dice, so a hidden defense cannot be mistaken for an early terminal. Terminal permission remains independent of roll disclosure. Existing safe GoalHistory owns scorer identity. No raw State or provider enters replication.

The shared Screen retains its Local set-piece identities and paired timeline. A coalesced Long comparison hides later defense totals/Narrative while revealing its attack prefix; early NoGoal retains the canonical attack terminal timeline and has no defense row/CTA. The existing safe central actor prompt, elapsed hold, displayed-score gate, Recovery feedback and Full-Time remain shared. General Near and Long capability is enabled; Penalty decisive resolution and Corner remain gated.


## Penalty resolution transport (Stage 7.24)

Penalty Direct and Panenka now reuse the existing canonical resolution through three typed PlayerIntents: ResolvePenaltyDirectAttackRoll, ResolvePenaltyDirectDefenseRoll and ResolvePenaltyPanenkaRoll. Each submits FMatchPlayPenaltyRollRequest through the shared HostPort to AuthoritativeSession; method selection only freezes the method. No CoreRules, FormulaResolver, Session or Coordinator gameplay implementation is duplicated or changed. A future authority-side PVE decision source can submit the same typed legal request; connection identity belongs to the Network adapter, not to Penalty rules. PVE itself is deferred.

Direct always draws attacker D6 then waits for defender D6. Canonical Finishing compares max(frozen taker Shooting, Passing) + raw attack D6 against goalkeeper Anticipation + raw defense D6 -3. The -3 is Defender.Modifier in the final total, separate from the attribute and unchanged die; ordinary TacticalPlayerModifier is zero. Existing FastSuppression precedes total comparison and an exact tie belongs to the goalkeeper. Panenka draws exactly one PenaltyPanenka D6: 1 NoGoal, 2–6 Goal, with no attributes, defender input or arithmetic Formula. Rules 13.4's Panenka outcome mapping does not imply a Formula comparison.

Both terminal transactions persist the frozen taker as scorer only on Goal, record GoalHistory once, then stop at TerminalPendingAdvance. Existing Advance consumes the taker/opportunity and runs canonical Recovery or MatchEnded; final Advance creates no Recovery or next D12. Safe projection requires two accepted Direct dice or one Panenka die plus independent exact-attack terminal permission. It reads persisted method before redaction, so a hidden defense roll cannot expose Formula/winner/history. LocalPlay keeps its normal disclosure defaults and deterministic development path.

Penalty joins the bounded carrier set-piece projection used by Near and Long; Corner remains gated. Shared Screen uses the existing single-die Panenka identity and Direct attack/defense timeline. It retains Panenka's static outcome hint during the reel, masks coalesced Direct defense totals during the attack event, and reuses displayed-score/ResultHold/Narrative gates without changing timing constants.
