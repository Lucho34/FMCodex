...
- The owner Controller uses the same generated RPC for listen host and remote. Client correlation/pending/ACK handling is shared across all five kinds, separate from intent-specific DEV display. No network gameplay resolver, generic workflow engine or second command registry is introduced.
...
- After both sides finish, the Coordinator runs its existing deterministic checks and stops at the first real player wait. With an available deployed attacker it stops at AwaitingCarrier / SubmitCarrier; the snapshot exposes the complete safe Carrier choices only to the acting viewer. SubmitCarrier uses the same shared port; later participant actions remain outside its allowlist. A canonical no-legal-Carrier continuation remains valid when that choice does not exist. The adapter never guesses or auto-submits the next player intent.
...
- MarkerSelection is diagnostic status only. No SubmitMarker/DeclineMarker, Runner, Helper, Skill or roll is admitted by this extension.
