...
## Cross terminal publication and Advance transport

This contract extends the earlier contest-only slice above: terminal disclosure and Advance are now transported for the ordinary Cross path. Earlier statements that these remained withheld/unnetworked describe that slice's boundary.

- Append AdvanceAfterTerminal to the existing generated owning-Controller ServerSubmitPlayerIntent protocol. Its eight tagged choice members are empty. MatchInstanceId, RequestId and ExpectedAttackSequence retain their common meanings; the server derives RequestingSide from the admitted connection. All eighteen kinds share one increasing per-connection namespace with an accepted forward delta of at most 1024.
- The exact canonical request is `FMatchPlayAuthoritativeAdvanceAfterTerminalRequest { AttackSequence, RequestingSide }`, classified PlayerIntent. Current terminal attacker alone can advance. Common validation precedes the adapter/shared port/serialized Session. There is no result, scorer, score, Recovery, opportunity, next actor or MatchEnd field in this request.
- The exact-attack terminal permission is granted only after accepted Cross Defense reaches the stable persisted terminal. BuildForViewer supplies redacted or disclosed facts; Goal/NoGoal, scorer, score and public history appear coherently in one revision. A/B score orientation remains fixed. The independent permission permits tests to prove committed authority score can remain hidden without mutating gameplay.
- The safe actor view exposes 下一回合 only when terminal facts are disclosed. Generic pending prevents repeated clicks and clears only after a correlated accepted ACK and its required/newer View, in either arrival order; rejection clears pending without inventing a new result. Stale Views and duplicate/unrelated ACKs retain existing behavior.
- Rendering completion is not a gameplay authorization token. A modified client cannot forge terminal outcome, but can request a canonically legal advance without waiting for local animation. No animation ACK/timer protocol is added; normal controls wait for disclosed facts.
...
## Marker decline, automatic absence and Recovery parity (Stage 7.20)

This supersedes the remaining Marker parity gap above. FMatchPlayAuthoritativeDeclineMarkerRequest has ExpectedAttackSequence and RequestingSide. Session.DeclineMarker is a PlayerIntent legal only for the defender in Resolution/AwaitingMarker with at least one canonical legal Marker option. The generated RPC derives Side from ParticipantRegistry and forwards the common envelope sequence; all intent-specific payload members are empty.

Zero legal Marker options use Session.ResolveNoLegalMarker automatically through Coordinator, never a decline RPC. Voluntary source/reason is DeclineMarker/MarkerDeclined; automatic source is ResolveNoLegalMarker, with DefenderHasNoDeployedPlayers or NoLegalMarker. Both delegate to CompleteMarkerGoal. Authority records a system team goal (no individual scorer), adds one GoalHistory fact, consumes the current opportunity and directly clears the attack. The next real input is Full D12, or none at MatchEnded. These early completions do not run Recovery and do not expose Advance. The earlier no-Carrier path has priority when both sides lack legal participants; later Skill absence is handled at its existing selection stage, not by Network preflight.

Safe public score/history and the completed system-goal notice are consumed in one stable refresh, without a fake Reel or delayed score-only phase. Normal rolled terminals retain all existing security disclosure and visible score gates. Marker options and 放弃盯人 coexist only for the acting defender; the waiting viewer has the mirrored Marker prompt without CTA. Marker selection uses the existing non-modal dock. Generic pending retains disabled safe candidates until the new View and preserves both ACK/View orders.

Ordinary optional-choice inventory is complete: DeclineMarker, DeclineRunner, DeclineHelper and DeclineSkill. FinishDeployment already permits declining further deployment; there is no separate ordinary Carrier decline, Skip or Cancel PlayerIntent. Local Corner confirmation cancellation is presentation-only and outside this family. The five ordinary tactics remain unchanged.
...
## Complete Near Free Kick Network family (Stage 7.22)

This supersedes the Near restriction in Stage 7.21. General player-facing common type entry and all legally reachable Near selection/resolution/terminal actions are enabled. Long/Penalty decisive resolution and Corner remain incomplete and gated; their optional selection milestone is retained.

| Wire tag / exact intent | Authoritative request | Actor and wait | Private draws / result |
|---|---|---|---|
| 45 ResolveShortFreeKickDirectAttackRoll | FMatchPlayShortFreeKickRollRequest | attacker; ShortFreeKick/Direct/DirectAwaitingAttackRoll | ShortFreeKickDirectAttack once; always defender wait |
| 46 ResolveShortFreeKickDirectDefenseRoll | same exact request | defender; ShortFreeKick/Direct/DirectAwaitingDefenseRoll with stored attack | ShortFreeKickDirectDefense once; canonical Finishing and persisted terminal |
| 47 ResolveShortFreeKickAngledRoll | same exact request | attacker; ShortFreeKick/Angled/AngledAwaitingRoll | ShortFreeKickAngledA then ShortFreeKickAngledB; atomic outcome-only terminal |

All three have empty kind-specific payload. No Side, dice, seed, Formula, outcome, scorer or result can be supplied by the client. Match, positive RequestId/sequence, closed shape, common <=1024 forward window, ledger, participant/connection identity and attack freshness remain shared. Wrong phase/family/method/side rejects before RNG. A consumed duplicate cannot draw again; a fresh ID cannot reroll an already accepted action. Provider failure may consume entropy but adopts no partial state, publishes no revision/disclosure and invokes no Coordinator. Retry uses a fresh ID in the same namespace. ACK and View may arrive in either order; both are required for accepted pending completion.

The narrow View reuses AcceptedContestRolls, Contest, SetPiece, Terminal and PublicGoalHistory. Near Direct stores accepted prefix indices 0/1 and exact roll purposes; Angled stores A/B at 0/1. Shared presentation events preserve Local identities: SetPiece.Attack/SetPieceAttack/0, SetPiece.Defense/SetPieceDefense/0, SetPiece.Short.Angled/SetPiecePairedA/0 and SetPiecePairedB/1. Stable identity includes attack sequence and owner. Near has independent exact-sequence disclosure bookkeeping, not a new request/retry authority. Formula/outcome/current goal facts are withheld until the final roll and terminal permission; scorer must match the canonical persisted GoalHistory entry.
