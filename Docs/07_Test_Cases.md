...
## Typed deployment transport regression contract

- DeploymentTransport tests exercise valid A/B deployments and choices outside the bounded advertised subset, including defender action while the attacker is unchanged. Assert the accepted State differs only by the canonical placement/turn rotation, with exactly one public revision and no entry/D12/all-provider entropy consumption.
- Reject wrong side, opponent/nonexistent card, unknown/illegal/occupied slot, empty/oversized payload, wrong match, stale attack, duplicate request, same gameplay with a new ID, unknown kind/internal action and kind/payload mismatch. Compare the entire authoritative State, all-provider entropy count and publication revision. Canonical gameplay replay is tested after ownership returns to the original side.
- Hold a genuinely legal deployment from N, advance a server-only fixture canonically to N+1 deployment, then submit the old choice with a fresh ID. It must be stale with no mutation or RNG; the valid N+1 request must still work.
- Verify the 1024 forward-window inclusive boundary, one-beyond/MAX_int64 rejection without ledger poisoning and the next normal ID succeeding. Full D12 and deployment cannot reuse each other's IDs. The ledger remains constant size.
- Test bounded UTF-8 name roundtrips (including Unicode and exact maximum), oversize/truncated/NUL/invalid-UTF8 failures, nested reflected field allowlists, shared HostPort variant mismatch, ACK/view in both orders, stale snapshots, repeated clicks and rejected pending completion.
- For real integration use the existing launcher with optional DeploymentSlice; invoke the same owning Controller DEV command through the generated RPC. Record remote B's legal payload/ACK, both public revisions and naturally cleared pending, plus actual Host/Client viewport images before/after. A non-Shipping automation-only DevProbeInvalidDeploymentCard uses the same correlation/RPC path and must reject without revision/RNG. Host uses the generated RPC when its deployment turn arrives.
- Network generalization is a readiness gate requiring NetworkPlay transport/RNG/bootstrap, full MatchPlayRuntime, full LocalPlay, full CoreRules and Editor/UHT verification. Actual process evidence does not replace USER two-window action/feel acceptance.

## Goalkeeper and deployment-completion transport regression contract

- GoalkeeperDeploymentTransport exercises both defending sides and a legal slot beyond the single offered option. Assert the wire has only SlotId, Session selects the server-owned unique GK, usage/current activation/placement/turn change canonically, CardUsage zones remain unchanged, and both public views agree.
- Reject wrong/attacking side, early phase, unknown/occupied/illegal slot, empty shape, wrong match, stale attack, huge ID, nonparticipant and ordinary-member contamination. A duplicate cannot activate twice; fresh-ID replay reaches canonical usage rejection. Opponent/non-GK CardId selection is unrepresentable in this slot-only DTO; nested reflected allowlists and closed-tag tests protect that boundary.
- FinishDeploymentTransport exercises either attacker order, both sides' finish flags, repeated same/fresh-ID requests and final phase closure. Early means an invalid canonical phase (for example before Full D12), not an invented minimum-card rule. A legal zero-card/no-GK finish must remain accepted.
- Hold real legal GK and Finish choices in attack N, advance canonically to N+1, then deliver those old choices. Assert complete State identity, zero entropy, zero publication and zero Coordinator calls on rejection; a valid N+1 command must still succeed.
- Count actual Coordinator invocations for all accepted and rejected new intents: exactly one after success, zero on rejection/duplicate. With an ordinary attacker deployed, both sides finishing must produce Resolution/AwaitingCarrier, no frozen Carrier/selected tactic/resolution session, one stable revision and cleared deployment actions. Do not auto-submit the next player input.
- Verify all four kinds share one ID namespace and common pending state: ACK-first, view-first, accepted convergence, rejected release, stale updates and repeated click suppression. The 1024 forward bound and no ledger poisoning remain covered by the existing deployment suite.
- Exercise safe Finish action projection, ordinary-route disclosure clearing, public finish/GK facts and reflected DTO allowlists. Validate GK bounded wire roundtrips/truncation and all new kind/member mismatches before HostPort.
- Real integration must use two processes and normal owning-controller commands through generated RPCs. Capture both viewports before/after GK and after Finish, record match/sides/requests/slot/revisions/pending/provider counts, and stop at the actual first unnetworked wait. The deterministic B-first launcher offers GK to Host A and Finish to Remote B; a complementary existing A-first authority fixture can exercise Remote B GK and an invalid-slot probe. No direct Session/Mode mutation or manual OnRep refresh qualifies as runtime transport evidence.
- Deployment closure requires focused GoalkeeperDeploymentTransport/FinishDeploymentTransport plus existing DeploymentTransport, PlayerIntentTransport, RNGPrivacy, NetworkBootstrap, full MatchPlayRuntime/LocalPlay/CoreRules and Editor/UHT verification. Real process evidence still requires final USER two-window interaction acceptance.

## SubmitCarrier transport regression contract

- CarrierTransport must reach canonical AwaitingCarrier and exercise both attacking sides, distinct legal choices and a complete real candidate set larger than the ordinary DEV sample. Compare the full authoritative State: with a legal Marker, only ActionPreparation.CarrierCardId and SelectionStage change; phase remains Resolution and Marker/tactic/resolution-session state remains unselected.
- Candidate projection copies the complete BuildForViewer SelectionOptions identity/order/name data, filters actions from nonacting viewers and clears withheld Carrier facts. Test the 19-option representational bound and all-or-none failure on overflow, duplicates or unencodable names; do not truncate legitimate choices.
- Reject wrong side, opponent/nonexistent/undeployed/GK identifiers, early phase, empty/oversized payload, wrong match, wrong sequence, nonparticipant, member/tag mismatch and internal tag. Compare full State plus zero Coordinator, entropy, entry/D12 provider and revision deltas. Carrier itself adds no separate Used/Ejected/position/tactical-match rule: canonical deployment and snapshot eligibility remain authoritative.
- Verify exact duplicate rejection, fresh-ID same-Carrier rejection and fresh-ID different-Carrier rejection after freeze. Capture a legal request in attack N, advance canonically to attack N+1 also awaiting Carrier, then reject the old sequence with a fresh request ID; a valid current request must still succeed.
- Test the bounded name codec (Unicode, exact limit, oversize sender, malformed/truncated/NUL/invalid UTF-8 receiver), all five tag/member combinations and complete reflected wire field allowlists.
- Verify five-intent shared RequestId namespace, <=1024 forward window, no high-water poisoning, repeated click suppression, ACK-first/view-first completion, stale view handling and rejected pending release without publication.
- Compare transport-neutral shared HostPort and Network adapter final State on identical fixtures. Both Local and Network Carrier branches must consume the same port; Host/Remote Controller submission must call the generated Server RPC without authority shortcuts.
- Count one Coordinator invocation per accepted Carrier and zero per rejection. With legal Marker availability, Steps is empty and the next wait is AwaitingMarker; no next PlayerIntent is executed. Without legal Marker availability, preserve exactly the canonical ResolveNoLegalMarker internal continuation and next Full D12 wait.
- Real evidence requires Remote B and Host A Carrier selection in separate processes, through normal owning commands/generated RPC, with candidate identities, selected identity/name, ACK, revision, pending, Coordinator/provider deltas and actual final authority wait. Capture both viewports before/after Carrier. Existing deployment/RNG/bootstrap suites and full MatchPlayRuntime/LocalPlay/CoreRules plus UHT/Editor build remain the readiness gate; USER interaction acceptance is separate.

## SubmitMarker transport regression contract

- Reach Resolution/AwaitingMarker using canonical deployment and Carrier submission for both attacking sides. Select distinct legal Markers and compare the entire State against the exact writer mutations: frozen Marker, participant-first flag and AwaitingRunner; preserve Carrier, absent Runner/Helper, empty Skill/ActionType and absent resolution session.
- Exercise the complete four-candidate prototype physical-half set and compare every safe RelatedCardId/name/order. Nonacting viewers receive no actionable options. Withheld route projection clears both participant facts. Test the independently justified 19-identity transport bound with all-or-none overflow, duplicate and encoding failure.
- Reject wrong Side, opposing/Carrier/nonexistent/undeployed identity, wrong physical half and legally deployed GK. Preserve canonical ownership, deployment and snapshot lifecycle checks; do not invent a separate Marker Used/Ejected gate.
- Reject early phase, mismatched typed members, malformed wire, wrong Match/sequence, nonparticipant, internal tag, zero/huge RequestId. Compare entire State and zero Coordinator, entropy, provider and publication deltas. Huge jumps cannot poison subsequent normal IDs.
- Verify duplicate and fresh-ID same/different Marker after freeze. Capture a request in attack N, complete that attack canonically, reach AwaitingMarker in N+1, and reject the old sequence with a fresh ID before any gameplay mutation.
- Verify all six kinds and all four choice-member combinations, exact reflected DTO allowlists, common pending suppression, ACK-first/View-first and rejection release. Local typed Marker and Network adapter must use the same shared port and equivalent final State; both owning Controller paths call the generated Server RPC.
- Preserve the canonical no-legal-Marker server-internal continuation, including its exact internal action and next attack wait. Marker acceptance with a legal Runner coordinates once, takes no internal step and leaves the next PlayerIntent unexecuted.
- Real transport evidence requires both Remote and Host Marker success through normal owning commands, natural owner replication, exact candidate/selection identity, actual authoritative post-Marker fields, ACK/revision/pending, Coordinator/provider accounting and both viewport captures. Full current network/runtime/LocalPlay/CoreRules and UHT/Editor validation support readiness; USER interaction acceptance remains separate.

## SubmitRunner transport regression contract

- Reach AwaitingRunner by canonical deployment, Carrier and Marker for both attackers. Accept first/last Runner, compare entire State with canonical freeze/AwaitingHelper, and prove one Coordinator/publication with zero entropy/provider delta.
- Compare every safe candidate ID/name/order, including all eight attainable prototype-board choices and both physical halves. Test independent 18-identity bound, whole-list overflow/duplicate/encoding failure, owner filtering and withholding.
- Reject wrong Side, opposing/Marker/Carrier/nonexistent/undeployed/GK identity, early phase, mismatched members/tags, malformed wire, wrong match/sequence, nonparticipant, internal/unknown kind and invalid RequestId. Compare full State and zero Coordinator/entropy/provider/revision deltas. Current attacker GK cannot be deployed by normal GK deployment: distinguish undeployed-GK network rejection from canonical non-GK snapshot validation. Do not claim an independent Used/Ejected or deferred-Skill physical-half gate.
- Prove duplicate and fresh-ID same/different frozen Runner rejection. Capture N, advance via canonical absence/no-Skill completion to N+1 also AwaitingRunner, reject N with a fresh ID. Huge jumps must not poison the next normal request.
- Exercise seven tags x 32 populated-member combinations, exact reflected payload/view allowlists, shared IDs, repeated clicks and ACK-first/View-first/rejected pending. Local typed Runner and Network must share the same HostPort and equivalent State; both owning Controller paths call generated Server RPC.
- With a legal Helper, stop at AwaitingHelper without internal actions or later player decisions. Without one preserve ResolveNoLegalHelper; without Runner preserve ResolveNoLegalRunner. Assert subsequent ResolveNoLegalSkill and next attack wait where applicable. DeclineRunner remains canonical but unnetworked.
- Real rendered evidence requires Remote and Host success, complete offers, selected identity/next wait, ACK/revision/pending and Coordinator/provider accounting. Keep automatic-continuation evidence separate from the Helper-wait route. Capture both clients before/after Runner and stop at the next PlayerIntent.
- Run NetworkPlay, MatchPlayRuntime, full LocalPlay/CoreRules and UHT/Development Editor build for shared-header networking readiness. Run launcher contract tests only if launcher changes; unchanged launcher still needs real two-process proof. USER interaction acceptance remains separate.


## SubmitHelper transport regression contract

- Reach Resolution/AwaitingHelper by canonical deployment, Carrier, Marker and Runner for both defending sides. Accept distinct legal Helpers and compare the entire State to the exact Helper freeze/bHasHelper/AwaitingSkill changes when Skill availability is nonempty. Preserve all earlier participants, empty Skill/ActionType, deferred selection and absent selected action/resolution session.
- Prove all four attainable prototype Helper candidates match BuildForViewer identities, names and order. Separately prove the independently justified 18-identity deck bound, all-or-none overflow/duplicate/invalid identity behavior, nonacting-viewer filtering and withheld selected facts.
- Reject wrong Side, opposing/Carrier/Runner/nonexistent/undeployed identity, frozen Marker, legally deployed GK and wrong Runner physical half. Helper has no independent Available/Used/Ejected or player-position gate; retain canonical deployment/snapshot checks.
- Reject early phase, mismatched tag/member, malformed bounded wire, wrong match/sequence, nonparticipant, internal/unknown tag and invalid request IDs. Compare full State and zero Coordinator, entropy, provider and revision deltas; huge jump leaves the next normal request usable.
- Test exact replay, fresh-ID same/different Helper after freeze and real attack N -> N+1 staleness while N+1 also awaits Helper. Eight tags x 64 populated-member masks and reflected DTO allowlists guard the closed protocol. Test all eight intents' shared ledger/pending namespace and Helper ACK-first/View-first/rejected pending with old/duplicate arrivals.
- Verify Local typed Helper and Network use the same port, equivalent final State and one Coordinator invocation after success, zero after rejection. Preserve no-legal-Helper internal continuation and separate voluntary DeclineHelper legality. Preserve ResolveNoLegalSkill automatic completion where no Skill exists; never select the next PlayerIntent.
- Real rendered evidence must prove Remote and Host generated-RPC submission, complete candidate/selected IDs and names, owner ACK, natural replication, revision/pending, Coordinator/internal-step and provider accounting. Distinguish actual Skill wait from no-Skill closure; capture both viewports before/after Helper.
- Run full NetworkPlay, MatchPlayRuntime, LocalPlay and CoreRules plus UHT/Development Editor build for this shared-schema readiness gate. Launcher contract tests apply if changed; an unchanged launcher still requires real two-process smoke. Technical automation does not replace USER PIE.

## SubmitSkill transport regression contract

- Reach actual Resolution/AwaitingSkill for both attackers using canonical deployment and participant commands. Accept every offered choice in representative fixtures and compare full State to canonical Skill writer output. Count one Coordinator/publication and zero entropy/provider delta.
- Verify production Session pinned-copy behavior: alter caller-owned rule data after Session construction and prove the same exact SkillId still resolves to the Session's original type/range and identical final State. Reflected wire allowlists must exclude rule objects, ActionType, modifiers, thresholds, Formula and RNG; shared production port must call SubmitSkill(Request), never the fixture overload.
- Reject wrong Side, unknown Skill, known unowned Skill, owned wrong-TP Skill and owned TP-compatible Cross with incompatible Runner. Reject wrong phase, match/sequence, nonparticipant, malformed or cross-populated member, internal tag and invalid IDs. Compare full State and zero Coordinator/entropy/provider/revision deltas; a normal next request must remain usable.
- Test duplicate, fresh-ID same/different frozen Skill and real N -> N+1 stale sequence while both attacks genuinely reach Skill selection. Exercise nine tags x 128 choice-member masks, bounded Unicode codec and invalid wire, exact reflected DTO allowlists, shared nine-intent ID namespace, <=1024 window, duplicate click and all ACK/View arrival orders.
- Compare every real safe option Id/name/order; RelatedCardId remains Carrier and cannot replace Id. Establish current content maximum two from all assignments/TP values and attain it canonically. Independently test theoretical bound three with a common Carrier and distinct Skills, all-or-none overflow/duplicate/invalid identity/Side failure, owner filtering and withheld selected Skill.
- Exercise all five tactic families' actual canonical post-Skill states. LongShot/CutInside/Cross await SubmitBranchIntent; PassControl/ThroughBall await their own initial route-roll PlayerIntent. Verify correct participant clearing/preservation, deferred=false, selected-action distinction, no resolution/route/terminal creation, zero internal Coordinator steps and no later player action.
- Preserve NoLegalSkill internal closure, separate voluntary DeclineSkill legality, optional Runner/Helper absence and unchanged earlier transports. Compare Local/shared-port and Network final State, and verify owning Controller uses generated RPC for both roles.
- Real rendered evidence requires Remote and Host successful Skill submissions, current candidate IDs/Chinese labels, actual server-pinned lookup, canonical final state, matching ACK/natural owner replication, revisions/pending, Coordinator/internal-step/provider accounting and both viewport captures before/after. Real process provider counters do not substitute for focused all-provider entropy assertions.
- Shared header/schema readiness requires full NetworkPlay, MatchPlayRuntime, LocalPlay, CoreRules and UHT/Development Editor verification. An unchanged launcher needs real two-process proof; contract tests are required if changed. Technical smoke is separate from USER two-window interaction acceptance.

## Ordinary branch family transport regression contract

- BranchTransport must reach each of the six legal Skill/branch combinations for both attackers. Compare complete Network and Local/shared-port State, canonical options/order/Chinese labels, correct actor, one publication/Coordinator and zero entropy/entry/D12 provider deltas. Do not infer family support from shared command identity.
- LongShot/CutInside must show exactly BeginResolutionSession and ResolveIntentDeterminedRoute internal steps, deterministic actual route, ReadyForResolution selection, RouteResolved session, no random route records and no player-roll progress. Cross must retain intended High/Low without a resolution session or actual route. Verify the precise next safe wait for every branch and stop before the next player command.
- Reject wrong Side, before-Skill phase, PassControl/ThroughBall active phases, incompatible cross-family values, unknown/None enum, all mixed members/tags, wrong Match/sequence, invalid IDs, internal kind and nonparticipant. Compare full State and zero Coordinator/RNG/revision deltas. Same DirectShot/DeadCorner values are legitimately shared by LongShot/CutInside; test their distinct authoritative interpretation rather than inventing a rejection.
- Test exact duplicate, fresh-ID same/alternate frozen branch and old sequence with a fresh ID after canonical attack N completion and a genuine N+1 branch wait. Fixture-only later roll and explicit AdvanceAfterTerminal may prepare N+1; these actions are not added to network transport.
- Exercise every byte value and truncated branch wire, ten tags x 256 member masks, exact reflected payload/view allowlists, ten-intent shared namespace, <=1024 forward window and normal request after a rejected huge jump. Test duplicate click, ACK-first/View-first/rejection, stale/duplicate ACK and old View without establishing branch facts from ACK.
- Compare every family safe list, prove two-choice bound, whole-list overflow/duplicate/invalid enum failure, nonactor filtering and withheld selected branch. Verify actual Local branch dispatch and both owning paths use shared port/generated RPC.
- Real rendered validation requires Host and Remote success, preferably different Skills, current complete options, chosen identity, server-derived Side, matching ACK, natural owner replication, revision/pending, exact internal steps/provider counters and next state. Preserve both viewers' before/after viewport PNGs and logs. Real provider counts must not be presented as fully instrumented entropy; focused tests supply full entropy assertions.
- Run complete NetworkPlay, MatchPlayRuntime, LocalPlay and CoreRules, normal UHT/Development Editor build and tracked/new-file whitespace checks for this shared-schema family. No clean build is required. Clear complete real-process evidence may satisfy this incremental DEV milestone without manual USER PIE; uncertain visible interaction/timing requires user validation.

## Initial-route transport family regression contract

- InitialRouteTransport must cover both acting Sides and every D6=1..6 for CrossHigh, CrossLow, PassControl and ThroughBall. Compare full Network/shared-port Local State, exact frozen Skill/intent, one committed initial D6, one provider boundary call, one Coordinator/publication, zero extra RNG/internal steps and each precise next wait. Assert the canonical command-origin classification is PlayerIntent.
- Exercise every ordered wrong-family pair, wrong Side, before Skill, Cross before Branch, unsupported/internal tag, nonparticipant, wrong Match, zero/stale sequence, zero/huge ID, all mixed choice members, exact duplicates and fresh-ID same/cross-family rerolls. Validate authoritative State and zero provider/Coordinator/revision changes on admission or legality rejection. Wrong Match and huge ID must not poison the next normal ID.
- Reach actual attack N route wait, complete N only through fixture-owned canonical contest/terminal/explicit advance, reach N+1 in the same family, and replay old sequence with a fresh ID. Verify stale rejection before RNG and correct current-side usability. These fixture-only later commands do not expand network transport.
- Inject secure entropy-source failure through the existing provider seam. Assert one failed provider attempt, no fallback or successful draw, no candidate-session adoption, no Coordinator/publication, correlated rejection and no automatic retry. A subsequent explicit fresh player request may succeed after the provider is repaired.
- For all three kinds test pending duplicate-click suppression, stale/duplicate ACK, old View, ACK-first, View-first and rejection. Validate closed thirteen tags x 256 member masks, reflected request/ACK/result field allowlists and actual empty-envelope serialization roundtrips.
- Test owner-only action projection, withheld route and initial-entry disclosure, exact safe typed route copying, missing/invalid/duplicate-record failure, no projection RNG and no post-route/Formula/terminal payload. Do not infer a route mapping from UI text.
- Verify canonical milestone setup for both sides/all three families: initialization and normal setup commands stop at AwaitingSkill with no selected Skill/action/session and no route draw. Refuse replay over an active match. Launcher tests prove flags reach only Host, default route RNG remains secure, and incompatible setup options fail clearly.
- Run complete NetworkPlay, MatchPlayRuntime, LocalPlay and CoreRules plus normal UHT/Development Editor build and tracked/new-file whitespace checks because shared dispatch, provider ownership, disclosure and resolution lifecycle meet here. No clean build is required.
- Real rendered code-side evidence must include generated-RPC success for Host and Remote across at least two families, actual safe D6/route, old/new current revisions, pending, provider/Coordinator counts and exact next waits. Cover the third family in real processes when practical; otherwise distinguish its deterministic automation-only coverage.
- The continuous Skill -> Cross branch -> initial-route/RNG block is a manual USER PIE milestone. Code-side PASS remains PASS WITH USER PIE REQUIRED until the user validates that short sequence, both viewers' convergence/disclosure and the absence of premature contest-roll controls.

## Cross contest transport regression contract

- Audit all four exact canonical requests and PlayerIntent origin. Test real adapter behavior against the direct shared port and compare complete authoritative State after both steps, preserving the four Local DEV invocation decorators.
- Exhaust High/Low x Attack D6 1..6 x Defense D6 1..6 for both attacking sides and both kept/flipped initial-route intent. Verify single-D6 provider entry, prefix order, safe disclosure, handoff, Formula outcome and terminal continuation; exercise canonical tie behavior without introducing a Network arithmetic implementation.
- After Attack assert Active / Resolution / ReadyForResolution / RouteResolved / PrimaryBranch, one PrimaryAttack record, no Defense fact, defender-only action, zero internal steps. After Defense assert two records, one ApplyCrossTerminalResolution internal step, zero extra RNG and TerminalPendingAdvance with attacker-owned AdvanceAfterTerminal still unnetworked.
- Assert score/GoalHistory is committed once only for canonical Goal, scorer is Runner, and pitch/roles/opportunity count/current attacker persist. BuildForViewer with terminal reveal withheld must hide the current score increment and GoalHistory; enabling its existing terminal permission must reproduce the authoritative score. Network carries no winner/Goal/scorer shortcut.
- Forge wrong Side, wrong actual route including flipped selection, premature Defense, before Skill/Branch/initial route, after completed contest/terminal, duplicates and fresh-ID same/opposite-route rerolls. All four kinds must reject actual PassControl, ThroughBall, LongShot and CutInside states before RNG.
- Reach actual attack N+1 through canonical terminal advance and setup, then replay fresh-ID held N attack/defense requests at equivalent current waits. Preserve the same generic client namespace during test setup; do not accidentally reuse an ID when testing sequence freshness.
- Cover wrong Match, huge/zero ID, zero/stale sequence, all eight mixed choice members, internal/unsupported commands and nonparticipants. Assert complete unchanged State, zero provider/Coordinator/publication effects, and no poisoning of the next normal request.
- Inject Attack/Defense secure entropy failure: one failed call, no adopted partial state/fallback/auto retry, correlated typed rejection and later explicit usability. Test ACK-first, View-first, rejected, old View, duplicate/unrelated ACK and duplicate-click suppression for all four actions.
- Validate seventeen tags x all member masks, reflected allowlists and actual empty-request serialization roundtrips. Safe projection requires ordered canonical purpose identities; duplicate/invalid/missing-prefix facts fail closed without inventing a result.
- Reuse the initial-route canonical test fixture for the actual second Cross consumer instead of duplicating deployment and participant setup. Keep previous initial-route cases intact.
- Run full NetworkPlay, MatchPlayRuntime, LocalPlay and CoreRules because shared dispatch, secure RNG, safe disclosure and terminal lifecycle meet here. Use normal UHT/Development Editor build and tracked/new-file whitespace checks; no clean build.
- Real two-process evidence must cover Host attacker/Remote defender and Remote attacker/Host defender, High and Low actual routes, owning generated RPCs and natural ACK/replication. Capture both viewports before Attack, after Attack/before Defense and after Defense, with exact request/Side/revision/pending/provider/Coordinator/state evidence.
- Clear role-reversal and disclosure evidence may satisfy this transport increment without USER PIE. The public terminal result/score plus explicit-advance block is the next manual milestone; uncertain presentation/disclosure still requires user acceptance.

## Cross terminal disclosure and explicit advance regression contract

This supersedes the previous Cross contest slice's still-unnetworked Advance expectation. Its explicitly concealed BuildForViewer checks remain required.

- Cover A/B attacker, actual High/Low, Goal/NoGoal, non-final/final and GK participation through canonical setup and the real connection adapter. Compare Network Advance with the same direct shared port's complete authoritative state. Check all deployed non-GK consumption, persistent GK usage, one opportunity, correct next actor and atomic CurrentAttack cleanup.
- Prove the disclosure boundary independently of persistence: with a real terminal Goal already committed, withhold the exact-attack permission and assert old public score, no current history/scorer/Goal and no normal Advance control. Restore permission and assert coherent result/scorer/score/history for both A and B without any State/provider mutation.
- Validate canonical GoalHistory scorer, NoGoal's absent scorer/current record, owner-only Advance, bounded history/Recovery projection and safe MatchResult. Missing/duplicate/conflicting decisions or malformed/overbound lists must fail closed. Assert no authority reads or RNG in projection.
- Non-final Advance must perform the existing Recovery operation only when its candidate count requires the provider, preserve score/history, clear current roles/Skill/branch/roll/terminal facts and offer next-side Full D12. Then submit that existing transport to prove the new CurrentAttack is created at the next sequence. Final Advance must skip Recovery, clear its fact, preserve final result/score/history and expose no next action or CurrentAttack.
- Recovery uses the existing stamina-weighted without-replacement provider: zero/one candidate requires no provider call; two or more candidates require one operation returning at most two cards. Inject failure on both first and second secure draw; assert no partial adoption, no Coordinator/publication and explicit later retry usability.
- Reject wrong Side, pre-terminal phases, duplicate Advance, fresh-ID repeat, actual later-attack stale sequence, wrong Match, huge/zero ID, zero sequence, all mixed payload members, internal command and nonparticipant. Assert ACK category and complete State/provider/Recovery/Coordinator/revision deltas, including that malformed admission cannot poison the next normal ID.
- Reuse generic pending for ACK-first, View-first, rejected, stale View, duplicate/unrelated ACK and repeated-click cases. Exercise all eighteen tags against all 256 payload masks, the shared <=1024 ID window, reflected DTO allowlists and actual empty-Advance serialization roundtrip.
- Keep normal input gated by disclosed facts, but separately prove local rendering completion is not a forged gameplay token: a modified client with a canonically legal terminal Advance cannot choose outcome or double-complete it.
...
Changes to the shared screen/public presentation contract require affected presentation regression; materially changed common Screen lifecycle justifies full LocalPlay. Player-facing networking milestone closeout uses the layered policy below, with full suites selected from actual touched behavior, launcher validation and incremental UHT/Editor build. Technical results never replace USER PIE for Reel/Formula/Narrative/score/next-turn continuity.
...
## Ordinary sequential contest and shared UI regression

FMCodex.NetworkPlay.OrdinarySequentialContestTransport covers all D6 pairs for each PassControl route and Feet, with both attacking sides. Compare full authoritative State with the direct canonical DTO/HostPort path and validate canonical Formula/terminal/scorer, one successful draw, exact prefix, one Coordinator pass, stable revision, independent terminal redaction, no early opportunity use, explicit Advance, Recovery and next Full D12. Keep PassControl's atomic Session terminal and Feet's Coordinator terminal as separate expected internal step counts.

The rejection matrix includes both owners at both phases, early Defense, same-ID duplicates and fresh rerolls, wrong family and actual BehindDefense/AntiOffside Feet requests, wrong phase/match, huge IDs without poisoning, every mixed payload member, internal tags, nonparticipants, provider failure/retry, and old AttackSequence at a real matching-family next attack. Assert ACK and unchanged State, entropy, provider counts, Coordinator and revision. Exercise real accepted ACK-first and View-first correlation through the same generic pending mechanism.

FMCodex.NetworkPlay.PlayerFacingOrdinaryUI shares its screen/backend fixture and prompt assertions with Cross UI tests. Cover all PassControl routes, Feet, both viewers, Goal/NoGoal and final/non-final progression. Assert exact family route-event kind, safe unresolved Formula rows, accepted event prefix, repeated-view dedupe, existing Reel blocking, single central CTA, mirrored status, authoritative/public/painted score timing, canonical Narrative and reused Full-Time. Preserve normal ThroughBall capability rejection and the server-only Feet milestone boundary.

Milestone evidence must run real independent Host/Remote processes through shared screen callbacks, generated owning RPC and natural replication, without directly applying Session commands or manually refreshing the screen. Record actual route/roll values, RequestId, revision, pending state, public and painted score, goal history, Recovery and next actor, with both viewer captures. Use layered regression: focused suites during implementation, directly affected presentation/runtime/rule suites after integration, and one final full NetworkPlay pass for shared protocol/schema/disclosure changes. Full LocalPlay requires material shared Screen/builder/backend/Reel/Formula/Narrative behavior changes; full Runtime requires shared runtime semantic changes; full CoreRules requires rule/Formula/route/scoring/legality/Recovery implementation changes. Select each full suite from the actual final diff, and report deliberately omitted suites with concrete reasons. Launcher changes require their assertions; reflected header changes require UHT/Editor build. USER PIE is required for final visual and interaction acceptance.

## ThroughBall conditional Network verification (Stage 7.17)

- `FMCodex.NetworkPlay.ThroughBallConditionalTransport.Matrix`: both actor directions; BehindDefense 1–2 gates and all 3–6 × Defense pairs; AntiOffside 1–6; both entries into every Direct pair with GK deployed/undeployed and every Chip face. Compare complete State with canonical typed Session/Coordinator execution; check exact RNG/provider/revision/Coordinator accounting, duplicate/fresh-ID refusal, scorer/history, independent terminal permission and real Advance/next Full D12.
- `ThroughBallConditionalTransport.Security`, `ClosedWire`, `WrongRoutesBranchesAndStale`, `Disclosure`: ownership, family/route/phase/choice/order, Match, sequence, RequestId window, mixed payloads, internal tag, nonparticipant, provider failure and common ACK-first/View-first/old-view/duplicate handling. Rejections preserve the complete gameplay State, Coordinator and revision. Provider failure attempts one provider/entropy call, commits no accepted roll and invokes no fallback. Security-hidden conditional rolls must also hide derived OneOnOne actions.
- `FMCodex.NetworkPlay.PlayerFacingThroughBallUI.GoldenPaths`: both sides, each materially distinct conditional terminal/continuation, Direct and Chip, non-final and final lifecycle, original central choices, mirrored actor/wait prompts, event dedupe, no score flash, authoritative Runner scorer, Advance clearing, Recovery, next Full D12 and Full-Time. Focused late/coalesced-view cases protect the extended accepted prefix.
- Affected checks include OrdinarySequentialContestTransport (PassControl/Feet), InitialRouteTransport, PlayerIntentTransport, RNGPrivacy, existing Cross terminal/advance and player-facing UI, Local ThroughBall/Formula/Reel/Narrative/control-surface tests, relevant canonical ThroughBall/OneOnOne tests and Runtime boundary/Coordinator tests.
- Completing this tactic and changing common wire/schema/disclosure justify one final full NetworkPlay milestone pass. A repair to shared Coordinator continuation also warrants broader Runtime coverage. CoreRules implementation and shared Screen lifecycle remain unchanged: affected canonical rule and Local UI suites are sufficient unless evidence shows broader impact. Narrow fixes after broad verification use their focused/affected tests.
- Real independent Host/Remote runs must cover BehindDefense → OneOnOne Direct, AntiOffside → Offside, and preferably Chip, BehindDefense OutOfPlay and the opposite actor direction. Use generated RPCs, natural owner replication, correlated ACK/View completion and rendered screenshots. Engine-captured evidence is not USER PIE. Keep the primary manual milestone short using the deterministic BehindOneOnOne launcher.
- Long synchronous player-facing automation batches must collect retired widget trees between fixture cases; normal engine GC ticks may not run during the synchronous body. This is test lifetime management, not a reason to raise production UObject limits or change project settings. Preserve completed result logs if an infrastructure interruption requires resuming unfinished suites.

## Specialized shot Network verification (Stage 7.18)

- `FMCodex.NetworkPlay.SpecializedShotTransport.Matrix`: both attack directions and both tactics; Direct D6 1–2 gates and all 3–6 × Defense D6 combinations, GK active/inactive; all 36 DeadCorner pairs. Compare full authoritative state with the canonical typed HostPort path; verify exact draws, one stable publication/Coordinator pass, terminal persistence, scorer/history and next Full D12.
- `FMCodex.NetworkPlay.SpecializedShotTransport.Security`: every new command and both actors, common envelope/identity failures, wrong checkpoint, first-draw and paired second-draw provider failure, ACK-first/View-first pending and fresh-ID stale requests at the actual corresponding next-attack checkpoint. ClosedWire exhausts all nine mixed-payload combinations; wrong-family/branch tests also verify dependent-fact redaction.
- `FMCodex.NetworkPlay.PlayerFacingSpecializedShotUI.GoldenPaths`: both tactics and sides, ImmediateMiss 1/2, Direct Goal/Miss and DeadCorner Goal/Miss, each through next attack and Full-Time. Use existing Screen callbacks, central branch controls, paired Reel order, Formula/no-Formula distinction, mirrored prompts, duplicate/coalesced View handling and displayed-score hold.
- Real independent Host/Remote milestones exercise LongShot Direct/DeadCorner and CutInside Direct/DeadCorner with opposite actor directions and Goal/non-Goal results. Require generated RPC, natural replication, correlated pending release, exact public dice/Formula/history, real elapsed reveal, Advance and screenshots. Automated captures do not replace USER PIE.

Regression scope: focused during development; affected canonical LongShot/CutInside/Formula rules, runtime network/typed boundary and shared Local presentation; one final full NetworkPlay pass is justified by appended public tags and completion of two player-facing tactics. Do not mechanically run full CoreRules, MatchPlayRuntime or LocalPlay when their global gameplay/Screen lifecycle implementations are unchanged. Milestone closure requires one short specialized-shot USER PIE. Evidence is ignored under Saved/Automation/Stage7_18; actual run outcomes belong in the Stage response.

## Shared shot modal prompt repair (Stage 7.18.1)

- The existing specialized-shot player-facing matrix now asserts the rendered central actor on both viewers, collapsed acting dock, concise waiting dock with no action-detail line, pending/rejected restoration, and normal D12/deployment prompts after Advance. Cover Direct attacker/defender/terminal handoffs, paired outcome-first results, both A/B owners, and late/coalesced views. During Reel/ResultHold, actor copy follows the safe accepted event without premature next-action text.
- The existing Local LongShot ownership case explicitly checks the optional network actor line stays collapsed in hot-seat. Run affected Local LongShot/CutInside/PassControl/InlineFormula and Network Cross/ordinary presentation cases because they share the widget or dock seam.
- This client-copy/layout repair adds no transport/schema, rules, RNG or lifecycle semantics. Focused and affected presentation verification plus a real Host/Remote screenshot smoke suffice; full NetworkPlay, LocalPlay, Runtime and CoreRules are not automatically retriggered. USER PIE checks actor clarity, concise waits, acting-dock de-duplication and legal CTA ownership only.

## Optional decline family verification (Stage 7.19)

- FMCodex.NetworkPlay.OptionalDeclineTransport.Matrix covers all three commands and both actors: legal adoption, wrong Side/phase, already-selected/no corresponding wait, replay, fresh-ID repeat, stale sequence at the corresponding next-attack wait, wrong match, zero/huge RequestId without poisoning, malformed members, unallowlisted tags, nonparticipant and both ACK/View orders. Compare complete State, publication, Coordinator and RNG boundaries on rejection; adopted fixtures verify exact next selection or direct completion and unchanged score/history.
- OptionalDeclineTransport.ClosedWire exhausts all 512 combinations of the nine payload members for each empty command and checks append-only tags/internal origin. OtherWaitsAndTypedBoundary rejects declines during genuine Cross terminal and ThroughBall OneOnOne, and rejects incorrectly typed HostPort variants.
- FMCodex.NetworkPlay.OptionalDeclineUI.SharedControls covers A/B acting viewers for all three choices, next attack and final match. Assert the existing rendered Chinese buttons, mirror prompts, waiting-viewer filtering, authoritative candidates retained/disabled while pending, duplicate suppression, rejection restoration, next Skill compatibility, cleanup, direct next D12 and reused Full-Time. Do not advance the test-only reveal clock after Full-Time, where interaction is permanently blocked.
- Retain canonical Runner/Helper/Skill absence and decline tests, Network no-legal Coordinator tests, runtime network-boundary tests, affected Local Runner/Helper/tactical-abandon controls and RNGPrivacy. Verify no legal Runner, Helper or Skill continues internally without a decline RPC.
- Real independent Host/Remote captures must cover the three shared UI callbacks, generated RPC, natural owner View replication, correlated ACK/pending completion, zero RNG delta, one Coordinator pass and exact next state. Record PIDs, match/sequence/request/revision and both viewers before/after; automated captures are not USER PIE.

Appended public tags and owner View schema justify one full NetworkPlay closeout after focused/affected tests. Do not automatically run full LocalPlay, CoreRules or MatchPlayRuntime: shared Screen/core rules/Session/Coordinator implementations remain unchanged. Preserve the five complete ordinary tactic paths and prior modal/ResultHold contracts. Evidence belongs under ignored Saved/Automation/Stage7_19; actual results and USER PIE decision belong in the Stage response.

## Marker optional completion and Recovery verification (Stage 7.20)

- OptionalDeclineTransport.Matrix / ClosedWire / OtherWaitsAndTypedBoundary now include Marker. Cover both actors, canonical legal candidates, exact Goal/scorer/history-once semantics, wrong Side/phase, already selected Marker, duplicates/fresh repeats, stale next-attack sequence, wrong match, RequestId window/no poisoning, all mixed payload members, internal tags/nonparticipants, typed HostPort mismatch and both ACK/View orders.
- OptionalDeclineUI.SharedControls includes both Marker directions and final/non-final completion, original 放弃盯人 button, waiting prompt/no CTA, pending candidates, immediate safe score/system-goal notice without a fake roll, clearing, next D12 and Full-Time. Fresh final Marker requests reject without mutation.
- MarkerDeclineTransport.OtherPhasesAndPrecedence and AutomaticAbsenceDisclosure cover deployment/other selections, both-sides-empty NoGoal priority and zero legal Marker automatic Goal. No client no-legal intent, no scorer invention and no dead wait.
- MarkerRecoveryUI covers canonical 0/1 return projection and normal/final Cross Advance, ordered two-card safe facts, both viewers' complete available/Used hand state, duplicate Advance with no Recovery rerun, nonblocking Local feedback reuse and a two-second world timer that does not restart on repeated refresh. Synchronous fixtures first activate pending timers before measuring elapsed intervals.
- Local ControlSurface.MarkerGoalNotification verifies the real Local synchronous adapter, same system-goal feedback, direct D12 availability and existing timer expiry. Affected Local recovery/control/feedback, canonical Marker/Recovery and runtime boundary tests protect unchanged first-class Local behavior.
- Real independent Host/Remote evidence must show legal candidates plus decline, both actor directions when inexpensive, generated RPC/natural owner replication/ACK release, coherent system Goal/history/score and next D12. Record normal terminal Advance separately for two-card Recovery, shared notification, stable hand identity/order and next D12. Automated screenshots are not USER PIE.

Public wire changes and completion of ordinary optional-choice parity justify one full NetworkPlay closeout after focused checks. Test-only follow-up assertions use targeted revalidation; unchanged CoreRules, Session/Coordinator and shared Screen lifecycle do not trigger their full suites. Keep evidence under ignored Saved/Automation/Stage7_20; actual outcomes and the USER PIE gate belong in the Stage response.

## Set-piece entry and selection verification (Stage 7.21)

- NetworkPlay.SetPieceSelection.TypeMatrix covers both actors, Full D12 9–12, all six type D6 results, exact state/actor, complete candidates, one private draw/publication/Coordinator pass, duplicate/fresh-ID reroll rejection, Corner boundary and independent security disclosure.
- MethodMatrix covers all six methods and both actors, frozen taker identity, exact typed options, RoutePending/SelectionStage=None, the first missing decisive PlayerIntent, no new dice/score/history and no unsupported capability. IllegalTakerAndEligibility checks opponent/GK/nonexistent IDs, complete 19-option ordering, overflow/duplicate/invalid projection and Near Angled eligibility from frozen authority.
- TakerReplayAndSharedNamespace covers both actors and all three non-Corner types, method-before-taker rejection, frozen taker identity, zero taker RNG, exact one Coordinator/publication, duplicate/fresh rewrite and the common taker/method RequestId namespace.
- SecurityMatrix exercises five commands × both actors, wrong Side/phase/family/match/sequence, nonparticipant, zero/out-of-window RequestId, mixed payload and generic pending/ACK/View orders. ClosedWire exhausts the thirteen-member union combinations, legacy rejection of new members, unknown tags/enums, bounded IDs, typed HostPort mismatches and reflected round trips. TransportSecrecyAndSharedPort pins the exact envelope, owner View and set-piece presentation field inventories, including the absence of raw participant snapshots, future resolution facts and Corner nominations.
- ReconstructedUsageAndProvider uses valid reconstructed available/Used/ejected card state without forcing route or result. The type provider still drives canonical mapping; existing Coordinator owns no-taker NoGoal. Verify provider failure/malformed return atomicity, safe terminal/Advance, and stale type, taker and matching-method requests at their actual waits in attack N+1. OtherWaitsAndSecureFailure covers other canonical waits and failure of the actual secure entropy source.
- SharedUI covers both viewers and all methods/Corner, original Full D12/type Reel, elapsed reveal blocking, duplicate View dedupe, hand draft before explicit confirm, typed callback, central actor, waiting no-CTA, no lower duplicate Continue, selected-method feedback and explicit unsupported boundary. Preserve affected Local FullD12SetPieceProduction, shared reveal/Formula/Recovery and ordinary presentation regressions.
- Capture real independent Near, opposite-actor Long and Penalty flows through generated RPC, natural owner replication and ACK release. Record PIDs, match/sequence/request/revision, both viewer screenshots during/after type reveal, canonical provider/Coordinator counts and boundary state. Corner automation is required; a real Corner smoke is useful when inexpensive. Do not equate automated screenshots with USER PIE.

Run focused checks during development and affected shared Local/runtime/CoreRules cases according to the final diff. Public envelope/View changes and a new player-facing selection family justify one full NetworkPlay closeout after focused evidence. Unchanged CoreRules, Session and Coordinator implementations do not automatically trigger their full suites. Full LocalPlay is not automatic when the shared Screen changes only the scoped set-piece safe adapter; cover its existing set-piece and affected reveal cases. Keep evidence under ignored Saved/Automation/Stage7_21. Final report must state actual results, omitted full suites, scope justification and USER PIE REQUIRED.

## Stage 7.22 — Near Free Kick resolution verification contract

Focused Network family: `FMCodex.NetworkPlay.NearFreeKick`. OutcomeMatrix covers both attacking sides, Direct 36 pairs and Angled 36 pairs per side, all attack D6 classes, FastSuppression, GK tie behavior, zero ordinary Tactical modifiers, exact provider/Coordinator/publication accounting, outcome/scorer/history, all disclosure prefixes, terminal reroll rejection, Advance/Recovery and next D12 freshness. Security covers all three kinds on both sides, wrong side/participant/match/sequence/window/payload/method/family/phase, consumed duplicates and fresh rerolls, first/second provider failure, fresh retry and shared ACK/View/rejected pending. ClosedWire checks reflection and exact HostPort variants.

SharedUI covers both sides, both methods, Goal/NoGoal and normal/final matches through the original Screen. Assert actor-only CTA, central actor, hidden duplicate dock controls, accepted Reel identity/dedupe, real elapsed hold, no early score/Narrative, A then B and helper lifetime, coalesced publications with contest-local index zero, existing terminal/Advance, nonblocking Recovery, next D12 and Full-Time. Prior SetPieceSelection tests retain type/taker/method/security coverage while updating only the newly complete Near boundary; other families stay gated.

Use real independent Host/Remote generated RPC and natural owner replication for Direct Goal (including defender handoff) and opposite-actor Angled NoGoal, through terminal/Advance/next D12. Collect both viewport screenshots, PIDs, matched instance/sequence, request/revision/ACK, safe accepted dice, score/scorer, provider and Coordinator evidence under ignored Saved/Automation/Stage7_22. Manually inspect the rendered actor/CTA, Reel, Formula/compact helper, result and Recovery screens. Preferred USER PIE: AngledGoal, own available eligible taker, choose 战术配合, watch both dice and delayed 1–0, Advance, Recovery and the other side's next D12.

One full NetworkPlay closeout is justified by three public roll capabilities, new Near disclosure/terminal publication and shared Network event queue integration. Affected Local FullD12SetPieceProduction, InlineFormula/RollReel and Recovery tests plus Near authoritative runtime/CoreRules regressions cover shared presentation and unchanged gameplay consumption. Full LocalPlay/Runtime/CoreRules are not automatic: no CoreRules, Formula, Session, Coordinator or Local rules/provider implementation changed, and the cross-contest accepted event array is populated only by the Network projection. Report actual passes/failures, any final focused reruns after fixes and omitted full suites; never equate code-side evidence with milestone USER PIE acceptance.

## Stage 7.23 — Long Free Kick resolution verification contract

Focused Network family `FMCodex.NetworkPlay.LongFreeKick`: for each actor, assert Direct early attack 1 and 2 individually, all 24 continuing attack/defense pairs and all 36 Power pairs. Cover exact Finishing operands (LongShot versus GK Positioning +2), zero TacticalPlayer modifiers, suppression/ties, provider/Coordinator/revision accounting, atomic terminal/scorer/history, all disclosure prefixes and independent terminal permissions, consumed/fresh-ID rerolls, Advance/Recovery and next D12 freshness. Early terminal requires exactly one roll and no defense/full Formula.

Security covers each typed command on both sides: wrong actor/family/method/phase, nonparticipant, Match/sequence/window, mixed payloads/internal tags, duplicates/fresh rerolls, first-die and paired second-die provider failure, unchanged State and owner Views on failure, fresh retry, shared ACK/View/rejected pending. ClosedWire checks reflection roundtrip and exact HostPort variants. Keep implementation-level using directives scoped inside automation namespaces so committed test sources remain safe under UE unity compilation.

Shared UI covers Direct Goal, Formula loss, early 1, early 2, Power Goal/miss on both sides through normal and final matches. Assert existing Reel/hold, actor/CTA ownership, Power A→B/helper lifetime, early terminal without defense CTA, coalesced comparisons without future defense/Narrative, duplicate event safety, no early score, canonical terminal, Recovery feedback, next D12 and Full-Time. Existing Near and selection tests retain their contracts; capability expectations extend only to complete Long.

Use ignored Saved/Automation/Stage7_23 for real independent Host/Remote generated RPC, natural replication and correlated ACK/View evidence: Direct Goal with defender, Direct early NoGoal without defender, opposite-actor Power Goal at exact threshold 11. Capture both screens, phases, score gates, scorer/history, provider calls and Advance/Recovery/next turn. Prefer one short PowerGoal USER PIE unless actual Direct handoff evidence needs additional visual acceptance.

One final NetworkPlay closeout is justified by new public tags, shared free-kick disclosure and a complete set-piece capability. Run affected Near shared UI/security, set-piece entry/selection, Local set-piece/Reel/Formula/Recovery, Long runtime/CoreRules and Formula tests. Full LocalPlay/Runtime/CoreRules are not automatic because no global gameplay implementation changes. After a test-only correction, rerun affected tests and retain the original full-run result; do not mechanically repeat the full suite.


## Penalty transport verification contract (Stage 7.24)

Use minimal sufficient evidence, per AGENTS.md. `FMCodex.NetworkPlay.Penalty.Contract` has four A/B × Direct/Panenka representative paths covering accepted typed transport, exact provider calls, safe prefixes, canonical scorer, Advance/recovery, duplicate Advance and next attack freshness. `Penalty.Security` groups the three intents × two actors into six cases: wrong phase/method/side, nonparticipant, mismatch/sequence/payload, atomic provider failure, fresh retry and ACK/View order. `Penalty.ClosedWire` verifies reflection roundtrip and exact shared HostPort variant. Existing common envelope matrices own exhaustive payload/window behavior; Penalty tests do not duplicate CoreRules dice/math matrices.

`Penalty.SharedUI` has four representative cases: A.Direct.Goal.Next, B.Direct.NoGoal.Final, B.Panenka.Goal.Next, A.Panenka.NoGoal.Final. Both viewers exercise shared screen callbacks, CTA handoff, actual elapsed reveal, repeated View safety, score/result gating, single-die hint, coalesced Direct events, normal Recovery/next D12 and final no-Recovery/no-D12. Canonical Penalty formula and Panenka 1/2/6 tests supply rule evidence independently of Network transport.

Affected verification selects changed Penalty selection expectations, closed-wire upper bounds, existing Local Penalty score disclosure, one Runtime Penalty end-to-end and representative Near/Long UI paths for the bounded shared projection. No default full NetworkPlay, LocalPlay, Runtime or CoreRules sweep. At most one real independent Host/Remote Direct path verifies generated RPC, natural replication, ACK/View, disclosure and terminal progression; A/B symmetry and Panenka differences remain automated. Screenshots are limited to a material visual observation. USER PIE must inspect both Direct (including separate 点球防守调整 -3) and outcome-only Panenka, actor/wait prompts, reveal/score timing and progression; engineering evidence is not milestone closure.


## Corner transport verification contract (Stage 7.25)

`FMCodex.NetworkPlay.Corner.Contract` uses A.High.Goal and B.Low.NoGoal to cover ordered locks/blind nomination, the explicit shared D6, count adjustment from authority, route choice/roll, two-die Formula, canonical known subtotals, exact provider/coordinator/publication counts, safe disclosure, scorer, duplicate Advance, consumption/recovery and next D12 freshness. Reuse `CoreRules.MatchPlayCorner.SharedD6MappingAndCandidateModifiers` for the canonical count/bucket matrix instead of reproducing that matrix in Network tests.

`Corner.Zero` covers attack-zero, both-zero, defense-zero with one and with three attackers, including RNG differences, real scorer, private preterminal facts, no fake dice/Formula, duplicate lock and Advance. `Corner.ProviderAtomicity` covers failure/fresh retry/deduplication at shared selection and internal automatic scorer. `Corner.SecurityAndWire` checks representative envelope/ownership/legality failures, all seven reflected payloads and exact HostPort variants, legacy payload closure, unknown tags and shared ACK-before-View pending.
