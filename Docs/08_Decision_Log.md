...
## 2026-09-05 — Goalkeeper deployment and canonical deployment completion

- Extend the same owning-controller reliable envelope/RPC/ACK to DeployGoalkeeper and FinishDeployment. Goalkeeper sends only bounded SlotId; Session derives the admitted side's unique GK. Finish has no gameplay choice. The ordinary and GK members are mutually exclusive by tag, and Full D12/Finish require both empty.
- Common participant/match/correlation/shape/stale/1024-window admission remains separate from intent legality. The canonical GK and Finish methods enforce current legal side, phase, usage and completion. No minimum placement count, GK prerequisite or attacker-only deployment rule is introduced.
- Local and Network reuse the existing entry/deployment HostPort, with one Coordinator pass after successful canonical mutation and zero after rejection. Safe views add one legal GK option, the pure canonical Finish availability result and public GK/finish facts; ACKs still carry no gameplay truth.
- Completing deployment stops at the first real player wait. With a deployed ordinary attacker this is AwaitingCarrier / SubmitCarrier. Only that high-level wait is disclosed; Carrier selection and later player actions remain outside this transport. Existing no-legal continuations remain server-owned.
- The optional B-first DeploymentSlice launch supports remote ordinary/Finish and host defender GK/Finish without new configuration or fixture controls. Default production randomness, LocalPlay behavior, request-window hardening and disclosure restrictions remain intact.

## 2026-09-05 — SubmitCarrier only over shared player-intent transport

- Add only SubmitCarrier to the existing four-intent network allowlist. Its exact player choice is bounded FName CarrierCardId; registry Side and validated AttackSequence reconstruct the existing canonical request. All tagged members remain mutually exclusive; the common RequestId/pending/ACK protocol is unchanged.
...
## 2026-09-05 — SubmitSkill only with server-pinned canonical rules

- Add only SubmitSkill as the ninth network intent. SkillId is the sole player choice; the server supplies registry Side and validates common sequence. Seven tagged choice members share generated owning RPC, request window, pending, ACK and publication. Local typed dispatch uses the same shared port and production Session.SubmitSkill(Request).
- Session's private constructor-copied Skill rules remain the authority. Network cannot supply rule objects, action type, TP bounds, participant requirements, modifiers, thresholds, Formula, RNG or next state. Known IDs still require Carrier ownership, TP and participant compatibility.
- Complete legal Skill options use safe SelectionOptions.Id, while RelatedCardId identifies Carrier. The bound is three from canonical snapshot capacity, independently of current content's maximum two simultaneous TP-compatible choices. Atomic representation failure replaces truncation; selected Skill follows existing viewer disclosure and central Chinese labels.
- Preserve the distinct participant-first outcomes: LongShot/CutInside/Cross -> AwaitingBranchIntent/SubmitBranchIntent; PassControl -> ReadyForResolution/ResolvePassControlInitialRouteRoll; ThroughBall -> ReadyForResolution/ResolveThroughBallInitialRouteRoll. No later choice or roll is automatic. Nonparticipating tactics clear Runner/Helper canonically.
- NoLegalSkill remains server-internal; DeclineSkill, DeclineRunner and DeclineHelper remain separate unnetworked alternatives. No branch, roll, Formula, set-piece or terminal transport is bundled.
- A narrow automation-only server flag FMCodexNetworkSkillSlice selects initial D12=6 through the existing authority RNG injection decorator, enabling a legal Remote Skill fixture. It does not alter content, decks, rules, ownership or later RNG. Existing DeploymentSlice keeps D12=4; default production remains server-private secure randomness. No launcher parameter, client wire field or production dependency is added.

## 2026-09-05 — Ordinary tactical branch family and milestone validation

- Transport all LongShot, CutInsideShot and Cross SubmitBranchIntent variants together as one canonical family. Branch payload contains only EMatchPlayElectiveBranchIntent Intent. Registry Side and the common sequence reconstruct the existing AttackSequence/RequestingSide/Intent request; Local and Network share the port and Session writer.
- Frozen authoritative Skill/type supplies semantics. LongShot/CutInside share DirectShot/DeadCorner; Cross uses CrossHigh/CrossLow. Cross-family incompatible values reject canonically. No client Skill override, second legality table or branch-to-Formula mapping is introduced.
- BranchOptions copies the complete safe list with global bound two, canonical order and existing Chinese labels, including 射向死角. Nonactors have no controls. Invalid representation fails as a whole; selected branch is public only under existing safe disclosure. Cross intent is distinct from its later actual random route.
- Preserve distinct continuations: shot families execute only the canonical BeginResolutionSession + ResolveIntentDeterminedRoute internal steps with zero RNG, then wait for direct attack/paired dead-corner input. Cross waits for ResolveCrossInitialRouteRoll with no session or route. No future roll/Formula/result transport is included.
- An explicit non-Shipping automation-only B-first branch fixture swaps intact Rodri/Doku deck order before initialization so the existing DEV deployment path can reach Remote CutInside. It changes neither canonical card contents nor defaults, and adds no client capability.
- Use medium-granularity stages: group 2–5 commands/variants when their transport, authority, payload, disclosure and lifecycle contracts actually align. Do not mechanically require one intent per stage or bundle unrelated subsequent commands.
- Incremental Network DEV stages may close without manual USER PIE when required automation plus real Host/Remote RPC, natural ACK/replication and clear viewport evidence pass. If visual behavior/timing remains uncertain, request USER PIE. The next preferred manual milestone closes a meaningful Skill -> Branch -> initial route/roll block. Automated technical evidence must never be described as user-performed acceptance.

## 2026-09-05 — Initial-route roll family and continuous Network milestone

- Group Cross, PassControl and ThroughBall initial-route requests together, retaining three strongly typed PlayerIntent identities. Canonical requests contain AttackSequence/RequestingSide; their network kinds carry no choice payload. Current frozen family, Cross intent and registry Side remain authoritative; no generic RollD6 or arbitrary Resolve RPC is exposed.
- Reuse the existing Session initial-route orchestrator and secure Network provider. Shared dispatch owns exactly one Coordinator call after successful adoption. Invalid/replayed/fresh-ID rerolls never draw; provider failure retains pre-command State without fallback or automatic resend.
- The committed initial-route D6 and actual route are legitimately public through BuildForViewer's exact-attack route reveal permission. Network DEV publishes these stable facts immediately, while contest rolls, Formula and terminal stay withheld. Future Reel timing is presentation; it cannot create a second gameplay/disclosure authority.
- A bounded typed route fact copies canonical safe outcome identities and reuses existing Chinese labels. Cross High/Low D6=1..4 preserves intended type and 5..6 flips it; PassControl and ThroughBall each retain their existing 1..2 / 3..4 / 5..6 mappings. No mapping/rule change is introduced.
- Every route outcome waits for the attacker's next typed roll with zero Coordinator internal steps and no additional RNG. Cross High/Low Attack, PassControl Attack, ThroughBall Feet Attack, BehindDefense P1 Attack and AntiOffside Attack are the resulting unnetworked boundaries.
- Allow an explicit non-Shipping server milestone fixture that prepares a valid AwaitingSkill state through canonical setup commands. Skill/Branch/Route under test still use owning generated RPC. Default launch remains normal; manual milestone route RNG remains secure; deterministic route values belong only to separate server automation injection.
- This first continuous Network Skill -> Branch -> initial-route/RNG block requires one manual USER PIE after complete code-side validation. A short Cross launcher mode replaces repeated unrelated deployment clicks; technical screenshot review must not be reported as user acceptance.
- Prefer the four Cross High/Low Attack/Defense commands as the next medium-granularity contest block: they share sequential side-owned single-D6 events and one canonical terminal contest. PassControl/ThroughBall Feet comparisons can form a subsequent compatible block; ThroughBall conditional/single-roll paths and LongShot/CutInside paired or direct-shot contracts require their own bounded audits. No later roll or Formula/result transport is implemented here.

## 2026-09-05 — Cross contest transport and post-defense boundary

- Group the four Cross High/Low Attack/Defense PlayerIntents as one transport family, retaining empty gameplay payloads and the existing per-connection RequestId/ACK/View contract.
- ActualBranch.Cross is the contest authority. Selected CrossHigh/CrossLow is immutable prior intent and can differ after initial-route RNG. Session validates actual route and sequential attacker/defender ownership before the existing secure provider.
- Shared dispatch owns one Coordinator pass per accepted input. Attack waits for Defense with zero internal steps. Defense permits the existing automatic ApplyCrossTerminalResolution step, which reuses recorded dice to resolve/persist the final finishing contest without extra RNG; it stops at TerminalPendingAdvance.
- Follow current terminal persistence rather than older Cross text describing a separate confirmation or immediate handoff. Score/outcome can be committed while pitch, roles, used opportunities and current attacker remain intact. AdvanceAfterTerminal is the next genuine PlayerIntent and is not transported here.
- Publish only the exact attack's accepted contest prefix through BuildForViewer, with two D6 fields and a safe Formula-complete flag. Do not add Formula totals, winner/Goal/scorer or complete internal objects. Preserve the separate terminal-reveal gate and safe scoreboard redaction.
- DEV diagnostics explicitly distinguish completed comparison from still-withheld terminal result/score, and expose no premature next control. Synchronized Reel animation remains later presentation work.
- Reuse the existing server-controlled Skill-wait fixture and secure provider injection seams for role-reversal evidence. Keep normal launcher defaults unchanged. Share existing canonical test setup only because the Cross contest suite is a real second consumer.
- Full regression and clear real High/Low role-reversal evidence can close this increment without another manual PIE; uncertainty requires USER PIE. The next medium-granularity contract is public terminal outcome/score/scorer disclosure plus explicit advance, followed by a meaningful complete-Cross manual milestone.
- Audit PassControl/ThroughBall Feet sequential comparisons separately from ThroughBall conditional continuations and LongShot/CutInside direct/paired-shot families; none is implicitly transported by this Cross work.

## 2026-09-06 — Cross public terminal disclosure and explicit advance

- Extend the prior contest-only transport boundary with safe Goal/NoGoal, scorer, score/history and the exact existing AdvanceAfterTerminal PlayerIntent. No Formula/internal terminal command becomes a client RPC, and no gameplay outcome or lifecycle field is client supplied.
- Keep terminal persistence distinct from public permission. Network uses a private exact-attack terminal permission after accepted Defense plus canonical terminal completion, then existing BuildForViewer redaction/projection. All terminal public facts share one owner View revision. Local production's narrative/score reveal timing stays unchanged; fresh stable Network DEV terminals disclose immediately.
- Reuse the shared Local/Network typed port, serialized Session and one Coordinator pass. Canonical Advance consumes all ordinary deployed non-GK cards and one attacking opportunity, preserves GK match usage, derives next actor/MatchEnd, performs non-final Recovery and clears CurrentAttack atomically. Non-final waits for existing Full D12; only that request creates the next attack. Final skips Recovery and offers no extra attack.
- Count Recovery privately by delegating to the existing secure weighted sampler. Zero/one candidate needs no provider; larger pools need one operation for at most two distinct cards. Failure at either draw leaves state unchanged. The safe result, not candidates/weights/randomness, may be disclosed.
- Bound public history to the current Network three-plus-three opportunity profile and Recovery to two cards. Malformed or overbound arrays fail as a whole. Future longer profiles must revise history capacity deliberately. Scorer labels come from safe canonical identity/display-name sources.
...
## 2026-09-06 — Ordinary sequential PassControl / Feet network closure

- Group the four PassControl/Feet attack and defense intents because they share explicit attacker-then-defender D6, connection ownership, empty payload, stale/retry safety and one finishing contest. Preserve actual canonical continuations: PassControl Defense atomically persists its terminal in Session; Feet Defense uses the existing Coordinator terminal step. Do not add client Formula/application commands or alter CoreRules.
- Generalize the existing Cross contest/terminal projection and exact-attack disclosure permission to these audited routes. Reuse bounded accepted events and existing PassControl/ThroughBall presentation values in the original shared screen. Keep server-private RNG, generic pending, mirrored status, event identity and the elapsed-game-time ResultHold repair.
- Enable all PassControl routes in ordinary player-facing Network skill selection. Keep general ThroughBall disabled until its conditional branches are complete; an explicit non-Shipping Feet milestone enables only its exact prepared attack through the existing server provider seam. Full ThroughBall remains incomplete, while Feet is a separately bounded completed branch.
...
## 2026-09-06 — Complete conditional ThroughBall through shared OneOnOne

- Group BehindDefense, AntiOffside and the shared OneOnOne continuation as one coherent missing tactic lifecycle. Add six exact roll kinds and one canonical DirectShot/ChipShot choice to the existing closed transport. Preserve registry-derived Side, common request ledger/pending and server-private RNG. Historical P2 and unrelated shot/set-piece families remain outside this contract.
- Preserve immutable primary roll records when entering OneOnOne, and disclose later rolls using distinct purposes and event identities. Bound the safe prefix to four post-route dice and five displayed events including the route. Extend the original ThroughBall surface and shared action adapter; do not create separate Network commercial widgets.
- Coordinator must respect the canonical completed single-roll BehindDefense gate: use existing terminal regeneration directly instead of the unavailable two-sided Formula. This corrects continuation, without changing CoreRules or duplicating gate probabilities. Formula/decision/terminal application stay server-internal.
- Security-hidden accepted conditional dice also hide their derived choice/actor/pending information. Preserve a public skipped-gate decision when only an unused Defense descriptor is removed. Security disclosure stays independent of local presentation suspense and the existing displayed-score gate.
- Enable full ThroughBall in general Network skill selection; remove the obsolete exact-attack Feet capability bypass. Preserve Cross, PassControl, Feet, LocalPlay independence, shared Screen pacing and explicit Advance/Recovery/MatchEnd. Provide server-only deterministic milestone fixtures and require USER PIE for the complete player-visible tactic.

## 2026-09-06 — Complete both specialized ordinary shot tactics in one Network family

- Group LongShot and CutInsideShot because their six genuine PlayerIntents share empty payload, frozen branch, connection-derived actor, freshness and the secure post-route provider. Preserve Direct's conditional attacker/defender handoff and DeadCorner's atomic attacker-only pair. No generic RollAnyShot command or duplicate gameplay implementation.
- Keep the existing difference in terminal adoption: LongShot uses the Coordinator terminal step; CutInside's canonical roll transaction adopts terminal atomically. Both use existing explicit Advance, Recovery and MatchEnd.
- Reuse safe Formula/outcome projection and the existing shared LongShot resolution surface for both tactics. Preserve zero-based shot event indices and the canonical DeadCorner.Outcome identity. No initial-route die, no fabricated DeadCorner Formula/Defense action, no client-computed scorer.
- Enable LongShot and CutInsideShot at the Network player-facing capability boundary after integrating every canonical Direct/DeadCorner continuation. Cross, PassControl and all ThroughBall routes remain supported. Optional declines, set pieces and connection resilience remain separate scope.
- Keep normal production RNG unchanged. Provide host-only non-Shipping deterministic milestones beginning at Skill, with player-selected branch and generated RPC rolls. Require focused/affected evidence, one justified NetworkPlay closeout and one short user-visible specialized-shot acceptance before Stage closure.

## 2026-09-06 — Central takeover owns actor and action messaging

Stage 7.18.1 supersedes the earlier dock-mirroring rule during the existing shared shot/method takeover: the central surface names the current actor on both network viewers and owns the phase/CTA explanation. The waiting dock keeps only its concise actor wait; the acting dock collapses. Terminal wait and pending submission copy belong to the center as well. Outside takeover, the original action/wait dock remains. This narrows prompt duplication without changing legal CTA ownership, safe disclosure, reveal timing, gameplay or transport. Local hot-seat retains its established convention. Rejection restores the existing diagnostic/fallback presentation.

## 2026-09-06 — Transport voluntary optional choices without normalizing their semantics

- Group DeclineRunner, DeclineHelper and DeclineSkill as three exact empty-payload PlayerIntents through the existing common RPC/ledger, shared typed HostPort, Session and Coordinator. This closes the three audited optional-choice Network gaps without replacing LocalPlay or duplicating rules.
- Preserve the canonical distinction between voluntarily declining an available legal option and automatic ResolveNoLegalRunner/Helper/Skill. Internal absence actions remain inaccessible to client RPC. Authority owns optionality and the next compatible Skill set.
- Preserve actual continuation: Runner decline removes Runner/Helper and returns to Skill; Helper decline keeps Runner and omits Helper before Skill in participant-first flow; Skill decline directly consumes the opportunity and clears the attack, reaching next Full D12 or MatchEnded with no terminal Advance/Narrative invention. No gameplay RNG or score/scorer/history change is introduced.
- Project the exact owner-safe decline action and reuse the unchanged Local Chinese buttons in the shared screen. Generic pending keeps authoritative options visible but disabled. Preserve modal prompt ownership, safe disclosure and elapsed reveal; no separate Network UI or decline pending engine.
- Validate the common protocol once with full NetworkPlay after focused/affected coverage. Human acceptance is based on actual visible handoff clarity, not a mechanical per-command requirement. Set pieces and network resilience remain independent bounded follow-up work.

The repository also retains the Local DeclineMarker / 放弃盯人 PlayerIntent outside the Network allowlist. Its canonical marker-absence Goal completion has a different score/disclosure boundary and is not included in the three-command scope above. Do not claim complete Local/Network voluntary-choice parity until that remaining path is audited and integrated.

## 2026-09-06 — Preserve Marker early Goal semantics and wire shared Recovery feedback

- Close the remaining ordinary optional-choice gap with exact empty-payload DeclineMarker (append-only kind 39), the common connection/RPC/ledger and existing typed HostPort. Voluntary legal-option decline and automatic zero-option ResolveNoLegalMarker remain distinct public/internal commands.
- Preserve the existing CompleteMarkerGoal path: a system team goal with no individual scorer, one persisted GoalHistory record and atomic direct opportunity completion. CD-069's early-selection exception means no TerminalPendingAdvance, extra Advance or Recovery. The earlier no-Carrier path retains both-sides-absent NoGoal precedence. Do not add a cross-stage Skill/Runner preflight or rewrite authority to fit an ideal manual fixture.
- Reuse the existing nonblocking Local resolution surface for a shared generic 规则判定进球 notification sourced from the already-public system GoalHistory fact. Show it with the coherent safe score at the next D12 wait; final state remains Full-Time. There is no persisted Marker cause after completion, so presentation must not invent a specific decline/no-legal narrative or a Carrier scorer.
- Recovery gameplay and safe DTO were already correct. Its missing Network production feedback is repaired by feeding existing safe entries into the Local BuildRecovery formatter and shared Screen. Retain the Local duration/behavior and use Network match/completed-sequence dedupe with a local world timer. No Recovery rule/provider change or new animation/modal is introduced.
- Ordinary optional-choice Network parity is complete across Marker/Runner/Helper/Skill. Follow-up work should group existing set-piece entry, Carrier and ShortFK/LongFK/Penalty method selection contracts; their specialized resolution and Corner sealed-nomination disclosure remain separately audited boundaries. USER PIE still owns visual/continuity acceptance.

## 2026-09-07 — Network set-piece entry and selection as one bounded family

- Admit five existing typed PlayerIntents through the shared RPC/ledger/HostPort/Session/Coordinator contract: type roll, taker, Near method, Long method and Penalty method. Keep server-derived Side, shared RequestId/freshness/pending and private secure RNG. Do not expose generic internal commands or decisive set-piece resolution.
- Preserve canonical D6 mapping (1–2 Corner, 3–4 Long, 5 Near, 6 Penalty), complete own available non-GK taker options and frozen-authority method eligibility. Specialized state is RoutePending, not ordinary Resolution. Near/Long/Penalty stop at the first missing decisive roll. Corner stops at sealed nomination entry and remains a separate architecture family.
- Reuse the existing Local screen, hand draft/confirmation, type Reel/ResultHold and method controls with bounded safe presentation values. Type security disclosure is exact-attack server permission; visible suspense remains the original elapsed-time presentation gate. No raw participant snapshots, future Formula/results or Corner secrets enter replication.
- Preserve automatic no-legal-taker NoGoal, already reached by Coordinator before another PlayerIntent. Reuse narrowly disclosed terminal facts and existing Advance; no client no-legal intent or new scoring implementation is introduced.
- Gate the partial selection milestone explicitly through a non-Shipping host-only provider fixture. Do not claim complete Near/Long/Penalty/Corner networking or enable a decisive CTA. Ordinary five-tactic and four-decline parity remains unchanged. Require focused/affected evidence, one justified Network closeout and short Near-flow USER PIE for visible acceptance.

## 2026-09-07 — Complete Near Free Kick without duplicating specialized resolution (Stage 7.22)

- Admit the three existing ShortFreeKick roll PlayerIntents as append-only empty-payload tags 45–47 through the common generated RPC, connection identity, RequestId/freshness/pending and typed shared HostPort. Keep every decisive Formula/terminal/Recovery/MatchEnd operation in its existing authority owner.
- Direct always requests the defender after attacker D6, including 1–2. Preserve max(Shooting, Passing) versus GK Handling +1, zero ordinary TacticalPlayerModifier, FastSuppression and GK tie. Angled is outcome-only (paired D6 sum >=9), with eligibility from frozen Shooting + Passing >=8; no arithmetic contest, defender action or fake Formula is added.
- Both terminal roll transactions already persist result/scorer/history atomically. Reuse existing Advance and Recovery/final lifecycle. A just-consumed taker can be returned by canonical Recovery; Available after Advance alone does not prove consumption was skipped.
- Publish only the exact accepted Near prefix and independently permitted terminal. Repair the narrow projection case where a persisted Direct Formula could survive hidden defense/terminal disclosure. Presentation retains the original elapsed Reel/hold/Narrative/score gates. Preserve Local contest-local identities and drain Network accepted events in array order across different contests, including equal local indices. During coalesced Direct attack reveal, hide later defense totals and terminal Narrative.
- Enable general Near capability after both methods are wired; retain Long/Penalty/Corner continuation gates. Reuse the original Inline Formula/compact pair surface, with central actor text sourced from the existing shared safe prompt. DEV host-only provider fixtures remain opt-in, non-Shipping and removable. Final visible closure still requires USER PIE; technical evidence is not user acceptance.

## 2026-09-08 — Complete Long Free Kick with canonical early terminal (Stage 7.23)

- Expose only the three existing Long roll PlayerIntents as closed empty-payload tags 48–50. Keep connection identity, request ledger/freshness/pending, typed HostPort, private RNG and serialized authority unchanged.
- Source confirms Direct 1–2 early NoGoal and 3–6 defender continuation. The FormulaType is Finishing; LongShot is the attacking attribute, not a replacement Formula identity. Preserve GK Positioning +2, no ordinary Tactical modifiers, suppression and GK tie. Power is atomic outcome-only 2D6 with threshold 11 and the existing 重炮轰门 terminology/helper.
- Reuse bounded Near/Long safe roll and terminal projection now that there is a second concrete consumer. Independent terminal permission requires one accepted die for persisted early Long NoGoal and two for comparisons/pairs. Determine the early shape before redaction; a hidden defender value must not change that requirement.
- Reuse the existing Local Screen, pair identity/timeline, central actor prompt, real elapsed hold and delayed displayed-score gate. Coalesced Long comparisons hide later defense results until their event; early NoGoal reveals through its existing attack terminal timeline without fake defense or full Formula. No new commercial widget or gameplay implementation.
- Enable complete Long alongside Near and retain Penalty/Corner gates. DEV fixtures are host-only provider overrides and do not choose method/taker, force outcome or bypass progression. Advance/Recovery/MatchEnd stay in existing authority; visible stage closure still waits for USER PIE.


## 2026-09-08 — Stage 7.24: Penalty typed transport and minimal verification

- Append only the three existing Penalty roll PlayerIntents at tags 51–53. Keep method selection separate, shared FMatchPlayPenaltyRollRequest/HostPort/Session, connection-derived Side, common request ledger and server-private provider. Future authority-side PVE may use the same legal intent contract; do not implement AI or human-only gameplay assumptions here.
- Canonical Direct always awaits a second, defending die. Use max(Shooting, Passing), goalkeeper Anticipation and separate Defender.Modifier=-3, raw dice unchanged, existing suppression/GK tie. Panenka is a single-die outcome mapping (1 miss, 2–6 goal), not an arithmetic Formula or paired roll.
- Extend bounded Near/Long projection and exact-attack disclosure to Penalty. Require two Direct dice or one Panenka die plus independent terminal permission; never infer early completion from a redacted defender value. Preserve existing frozen-taker scorer and Advance/Recovery/MatchEnd authority.
- Reuse Local shared Screen/event identities, central action/wait ownership and displayed-score gate. Preserve the static Panenka hint through reel/settling; do not add a second die or change reveal durations. Corner is deferred.
- Consolidate AGENTS verification into a durable minimal sufficient budget: no dice-matrix duplication in transport, at most one real two-process path per normal stage, screenshots only when materially useful, and no full NetworkPlay absent an actual global/shared semantic change or milestone closeout. Selected focused/affected evidence must be named and justified. Stage 7.24 remains subject to USER PIE and user manual commit.


## 2026-09-08 — Stage 7.24.1: Pending set-piece subtotal parity

- Security redaction may remove a roll-dependent CurrentTotal while retaining the separately projected public KnownSubtotal. Missing CurrentTotal must not turn an already-known subtotal into `?`: the shared opposed set-piece presentation selects disclosed CurrentTotal first, then existing KnownSubtotal, then unknown. A resolved Formula still supplies the final result. No attribute/modifier arithmetic is added to Widget or Network code, and no disclosure field or permission changes.
- Penalty Direct, Near Direct and Long Direct use this same projection. Ordinary tactical BuildFormulaRow already has the known-subtotal fallback. Panenka, Near Angled and Long Power remain outcome-only and do not enter the opposed Formula-row block. LocalPlay keeps its existing CurrentTotal-first display.
- Verify the three visible Penalty Direct states and one hidden-defense terminal prefix through existing shared UI automation; use one existing Local Near formula test for affected behavior and LocalPlay preservation. No broad suites or additional real Host/Remote run are required for this presentation-only repair. USER PIE still owns the final visual re-check and Stage 7.24 closure.


## 2026-09-08 — Stage 7.25: Complete Corner through existing authority

- Audit confirms seven genuine Corner PlayerIntents, including explicit attacker shared-selection and route rolls. Append closed wire tags 54–60 and dispatch exact existing request variants through the common HostPort. Keep candidate legality/order, count adjustment, route mapping, Formula, scorer and progression in existing authority; do not clone Cross or introduce a generic roll/workflow command.
- Preserve ordered 0–3 candidate drafts and Local underfull confirmation/back. Before defender lock, only the attacker's own viewer receives its sealed ordered identities. A bounded safe selection DTO supplies both shared Screen consumers without replicating bindings/snapshots. Independent accepted participant/route/terminal permissions prevent hidden derivatives from entering client views.
- Attack-zero, including both-zero, remains no-RNG NoGoal. Defense-zero with attackers remains a real-scorer Goal under CD-093: single candidate requires no draw; multiple candidates use private CornerAutomaticScorer, never a client event. Both skip participant/route/Formula presentation and reuse canonical terminal/Advance/Recovery/MatchEnd.
- Enable general Corner capability alongside Near, Long and Penalty. Reuse Local Corner board/controls, Formula known-subtotal fallback, distinct event identities, coalesced reveal queue and displayed-score gating. DEV launch only overrides server provider results; users still choose candidates/order/route. Future PVE remains an adapter to the same authority, not part of this stage.
- Use a small Corner contract/security/UI set plus selected canonical mapping, safe-view, Local Corner, prior Penalty and closed-wire regressions. One representative real process pair and at most one material screenshot supply transport/presentation evidence. USER PIE and user manual staging/commit remain required for milestone closure.

## 2026-09-08 — Stage 7.26: Default Network launch and milestone boundary

- Normal project-local Network launch now selects the existing shared player Screen without opting into a milestone or deterministic RNG. Default viewport is 1600×900; explicit dimensions and optional DEV diagnostics remain available. Legacy diagnostic fixtures retain their original selection. No CoreRules, authority, wire, disclosure or Screen lifecycle changes are required for this entrance repair.
- The current capability inventory supersedes historical partial-family descriptions: five ordinary tactics and Near/Long/Penalty/Corner all return through the shared terminal/Advance/Recovery/MatchEnd authority. Typed wire tags remain 1–60. LocalPlay and a future authority-side PVE decision source retain the transport-neutral HostPort boundary.
- Networking milestone closure warrants one integrated NetworkPlay suite plus selected Runtime safe-view and LocalPlay lifecycle anchors. Reuse accumulated real family evidence; only the repaired normal launcher needs one new pair. Stage 7 acceptance does not imply Steam, Dedicated Server, reconnect/timeout or WAN readiness. Known teardown-only PitchSlotWidget ensure stays deferred; final user acceptance and manual staging/commit remain user-owned.


## 2026-09-10 — Stage 8.1A: Target-derived shell spec and fixed tactical geometry

- The AI TARGET has no PSD/Figma source. Use the [1920×1080 implementation spec](UI/Match_Shell_Implementation_Spec_1920x1080_v1.md) and measured actual UE captures to drive fidelity; do not wait for layered source files or treat “looks better” as acceptance. Reference measurements, chosen implementation parameters and frozen constraints must remain distinguishable.
- The visual target is a perspective stadium environment around a stable near-orthographic tactical board. Preserve current playable canvas, slot positions/hit testing, deployed-card placement, corrected field lines and authoritative 后场/中场/前场 semantics. Improve enclosure, advertising-board treatment, lighting and foreground through separate presentation layers.
- Stage 8.1A covers shell, Header/score, roster containers, pitch presentation, action dock and shared button/material/decorative language. Compact card portrait/name/position/rarity/pips and Full Card remain frozen; their target differences belong to Stage 8.2, not 8.1A fidelity failure.
- Dynamic identity, portraits, score, rounds, tactical points, phase, legal actions and localized text remain truthful and UI-rendered. Keep existing fonts and localization architecture. No fake timer, AI club crest, sponsor identity or text baked into textures. Status remains noninteractive; Finish Deployment is Primary and Tactical Reference is Secondary.
...
## 2026-09-10 — Stage 8.1B: One player master and purpose-specific runtime art

- The user confirms Stage 8.1A passed USER PIE and was manually committed at `2d33963bdb36f3ea41ba0c6ce2738cd849e82aa1`; its shell/Header/Dock scope is CLOSED. Header round-indicator micro-polish stays future backlog.
- Consolidate the existing [Shared contract path as Player Art Architecture v1](UI/Shared_Portrait_Art_Contract_v1.md), the single authority for future master/derivative/import/migration policy. Hand layout and current legacy reproduction records remain intact; older permission for independently authored Hand/Full images is superseded for future production.
- Freeze one 1024×1536 opaque sRGB RGB source master per stable PlayerKey outside `/Game`; derive Shared 512×768, Hand 192×128 and Full 768×1152. Pitch reuses Shared; drag reuses Hand. Use existing fixed Widget crops and geometry, with minimal normalized source-space Hand/optional Full crop metadata and no localized text or per-player Widget parameters.
- Full 768 follows a bounded real UE card comparison of Saka, Rodri and Raya at 1920×1080 and 2560×1440. At 1440p, 512 visibly smooths hair/beard and kit weave; 768 is selected for detail retention, not extra safety. Transient review texture evidence locks resolution only; imported BC7/mips and actual player acceptance remain the Stage 8.2 gate. No new art, production imports or routes were changed.
- Three desktop derivatives remain appropriate because Full and Shared have different selected dimensions. Logical roles stay platform-independent: BC7 is the desktop reference recipe, with future ASTC/ETC/DeviceProfile choices from the same masters. Explicit same-player Shared/Full aliasing is only a future platform option when outputs/settings truly match.
- Avoid treating smaller pixel dimensions as guaranteed memory savings: selected Full BC7/full-mip payload exceeds the current opaque-BC1/no-mip assumption. Hand currently also synchronously loads Shared, Full also loads Hand, and hidden detail retains references. Stage 8.2 must prove mode-appropriate acquisition and bounded residency; this architecture stage does not change loading.
...
## 2026-09-12 - Stage 8.3C source-gated Batch 2, v1.1 preserved

The user authorizes eight named Batch-2 candidates under frozen Player Card Family v1.1, with partial completion allowed. Seven pass the repository source gate: Arsenal Saliba, Timber, Odegaard, Rice; Manchester City Dias, Bernardo, Foden. Cherki has no source and remains blocked; Ake's prior missing status is unchanged. Canonical enablement grows from 11 to 18, with all new roles PENDING USER PIE and the original 11 acceptance records retained.

Use one existing same-key fictional source per player across Master/Hand/Pitch/Full. Bernardo's `_01` candidate replaces his unsuitable HeroBust candidate for all canonical purposes; no cross-player or per-purpose source substitution. Timber alone needs bounded Hand/Pitch top `.115` crop metadata to remove a crown extraction remnant. No frozen generator, family geometry/style, number/bio data, gameplay or networking change. Exact paths, hashes and reasons live in manifest/provenance and the related art contract; review evidence is ignored Saved output. No Batch 3 or automatic staging/commit.

## 2026-09-13 — Stage 8.3C.1 drop completion and global Full bio clearance

The user clarifies that the final drag/drop disables both Hand and Pitch Full hover. Reproduced through real Slate pointer routing with the ten-card canonical example. On the final Gvardiol drop, synchronous hot-seat Rack refresh replaces its named node with White; the replacement has no legal destinations and previously omitted completion forwarding. The UMG Operation ends, but the shared screen's drag-suppression flag remains set. Rack completion forwarding is now unconditional; starting a drag remains legality-gated. No card-count, player-specific or authority workaround is introduced.

All Full biographies share width 90, horizontal padding four and 12-point facts, including legacy/missing/future routes. Existing top/right anchors, vertical capacity, portrait/crop and every other frozen surface remain. This is a local Family v1.1 stability/clearance amendment, not a new family version. Batch-2 migrations, provenance, explicit routes and source-missing Cherki/Ake remain intact. Stage 8.3C/8.3C.1 remains pending USER PIE; no acceptance is inferred from engineering evidence.

## 2026-09-13 — Stage 8.3C + 8.3C.1 USER PIE acceptance and status closeout

The user explicitly confirms USER PIE PASS for Stage 8.3C and Stage 8.3C.1. This supersedes the pending acceptance recorded in the preceding two checkpoints; their historical implementation records remain unchanged. Batch-2 Saliba, Timber, Odegaard, Rice, Dias, Bernardo and Foden are accepted across Hand, Shared/Pitch and Full. Canonical coverage remains 18; Timber Hand [0,.115,1,.5] and Pitch [0,.115,1,.55] metadata exceptions are accepted, with Full [0,0,1,1]. Cherki and Ake remain SOURCE_MISSING and non-canonical. Player Card Family v1.1 remains accepted/frozen; Batch 3 has not started.

The final-deployment hover repair is accepted: synchronous Hand-node replacement previously lost drag-end forwarding and left bDeploymentDragActive set, blocking Hand and Pitch Full hover. All rendered cards now retain completion forwarding while drag-start legality stays unchanged. The global Full bio repair is also accepted: canonical width 96 to 90, legacy width 100 to 90, horizontal padding four, text lane 82, values 12-point and labels nine-point, with existing top/right anchors and no per-player offsets. No further blocking repair is required. Bernardo residual face/bio proximity remains a non-blocking P2 future family-polish item.

This closeout synchronizes only current acceptance metadata, documentation and test expectations. Art bytes, hashes, paths, recipes, crop values, revisions, source identities, canonical enablement, routing and all production code remain unchanged. New or changed art still returns to PENDING USER PIE; unchanged accepted outputs retain acceptance. Staging and commit remain manual; Stage 8.3D and Batch 3 are not started.


## 2026-09-13 — Stage 8.3E real biography policy and missing Master authoring

The user now explicitly authorizes live public real-world birth date, height, weight and nationality for the same-named professional footballers. This changes the earlier repository-only bio authoring restriction; it does not require realistic portrait likeness. Original fictionalized portraits remain an accepted art policy, with identity defined by canonical PlayerKey ownership and consistency across one Master and its three derivatives.

Fill only absent presentation-sidecar fields. Prefer official club profiles, official competition profiles, federation/national-team profiles, then one reputable database provider if the higher classes do not supply the field. Record exact URLs and decisions in ContentSource/PlayerContent/PlayerBioProvenance.json; archived club measurements are labeled as archives rather than newly measured values. Unresolved conflicts stay absent. Preserve the 64 existing authored fields. No roster/team/position, shirt-number, serial, skill or balance import from live websites.

Stage 8.3E recovers 95 of 96 missing fields: 159/160 are present. Rayan Ait-Nouri weight remains absent because no official metric weight was recovered and ESPN's regional metric/imperial records disagree. Existing fixed-row localized missing-value presentation remains. The original approved workbook plus authored config regenerate CanonicalPlayerContent.json; provenance is offline authoring evidence, not a runtime lookup.

Generate exactly one new original Master each for NathanAke and RayanCherki with integrated image_gen. Same City-family sky-blue kit and frozen v1.1 pipeline; no existing 18-player art, generator, shell, crop, hover or gameplay change. Ake needs only source-space crop metadata to retain long-hair clearance: Hand [0,.025,1,.53], Pitch [0,.025,1,.58], Full unchanged. Cherki uses defaults. New canonical-only routes may resolve without a fabricated legacy entry. Canonical coverage increases from 18 to 20; the two new families are PENDING USER PIE, with prior 18 acceptance untouched. Batch 3 remains unstarted and staging/commit remain manual.


## 2026-09-13 — Stage 8.3E.1 weight resolution and explicit visual acceptance (current)

This supersedes the preceding Stage 8.3E missing-weight and pending-acceptance snapshot. For the final Ait-Nouri weight, the user authorizes wider current/archived research and a quality-based published-value choice, rather than leaving data blank merely because reputable sources differ. Group localized mirrors and possible syndicated copies into source families; normalize units for comparison without averaging or inventing canonical values. Prefer official current/recent evidence, then independent reputable consensus, then a defensible quality/freshness winner. Only extreme unresolved conflict may remain missing.

Select 70 kg from the FFF official retained national-team profile: https://www.fff.fr/equipe-nationale/joueur/8926-ait-nouri-rayan/fiche.html . The live page identifies Manchester City and includes a 2026 honour, but does not date the weight measurement. BeSoccer and BDFutbol also publish 70 kg. ESPN's 68 kg and 152 lb (~68.95 kg) are one provider's differing representations; Soccerbase publishes 69.84 kg and AS publishes 80 kg. FFF official priority wins (CASE A); the value is not an average or derived from physique. Exact URLs, retrieval date, freshness limits, alternate values and exclusions are recorded within the existing PlayerBioProvenance.json Ait-Nouri entry. Only weightKg changes in authored/generated player data; all 40 players now have all four bio fields (160/160).

The user has explicitly visually accepted NathanAke and RayanCherki. Synchronize manifest, provenance Hand/Shared/Full role statuses, generation-record current status and focused expectations to USER PIE ACCEPTED. Their 14 art files, all prior 18 families, hashes, recipes, crop metadata, routes and the frozen family UI remain unchanged. NEW OR CHANGED ART -> PENDING USER PIE; UNCHANGED USER-ACCEPTED ART -> remains accepted. Canonical count stays 20/40; Batch 3 has not started. No additional USER PIE gate for this data/status closeout; staging and commit remain manual.


## 2026-09-13 — Stage 8.3F shared missing-portrait silhouette refinement

Implement only the Stage 8.3F.0 approved global fallback refinement. The shared MissingPortrait Slate branch uses one U=min(W,H) geometry rule to connect the faceless elliptical head through a short .11U neck to symmetric shoulders and a lightly tapered torso. Preserve the prior head placement, torso envelope, navy background, neutral colors, clipping and containers. A contiguous vertex strip prevents detached gaps and avoids double alpha at joins; no PlayerKey or Hand/Pitch/Full tuning branch, new resource, material, face, kit or baked number is introduced.

Full portrait/bio safe-zone remains DEFERRED; Hand BalancedBust_v2 and Pitch QuietPitchBust_v2 remain KEEP CURRENT. Player Card Family remains v1.1. All accepted canonical art, crop metadata, provenance, routes, loading, number/serial contracts, layout and gameplay/networking remain unchanged. No Batch 3. Stage 8.3F.1 records explicit USER PIE PASS: the shared fallback refinement is USER PIE ACCEPTED on Prototype.ManchesterCity.MarcGuehi across Hand, Pitch and Full. Hand remains readable, Pitch has coherent quiet head/neck/shoulders, Full no longer reads as detached ellipses, and the fallback stays faceless and identity-free. No further visual repair is requested. The accepted rendering implementation remains unchanged; prior focused GlobalDataAndFallback and incremental Editor build PASS are retained without rerunning them for this documentation-only closeout. The existing 20 player-art families remain USER PIE ACCEPTED, with their acceptance records untouched. Staging/commit remain manual; pre-commit audit is a separate next step.
...
## 2026-09-13 - Stage 8.4 Batch 3 production, pending visual acceptance

Produce exactly eight canonical families: Gyokeres, Ait-Nouri, Havertz, Guehi, Eze, Marmoush, Zubimendi and Reijnders, four per team. Gyokeres/Ait-Nouri/Eze/Zubimendi reuse inspected same-key legacy sources. Havertz/Guehi/Marmoush/Reijnders use newly generated original fictional portraits; real-player likeness is not a gate. Exact generation/source evidence is in ArtSource/UI/PlayerMaster/Stage8_4_Generation.json. Eze uses existing bounded Hand/Pitch crop metadata to remove crown extraction residue. Marmoush is regenerated after his initial collar disconnected the torso under the frozen extractor; the selected light-collar Master passes the same recipe. No global pipeline or per-player UI rule changes.

Canonical coverage becomes 28/40, 14 per team. The existing 20 remain USER PIE ACCEPTED; all eight new families and 24 purpose outputs remain PENDING USER PIE. Each has one Master and independent Hand/Shared/Full derivatives. Full safe-zone remains DEFERRED, BalancedBust_v2 and QuietPitchBust_v2 KEEP CURRENT, family version v1.1. Bio 160/160, configured numbers and independent serials remain unchanged. Guehi's former missing-art test expectation moves to still-unmigrated JohnStones without removing fallback/rebind coverage. All eight require USER PIE before acceptance; staging and commit remain manual.

## 2026-09-13 - Stage 8.4R targeted small-card repair and permanent preflight

USER PIE did not accept Batch 3: the four City candidates required individual small-card comparison and Havertz Hand needed composition review. Replace only Marmoush/Guehi Masters for weak symmetric enclosed shirt/bust presentation. Apply modest Hand-only crop metadata to Havertz, Ait-Nouri and Reijnders; their Master/Pitch/Full stay unchanged. Gyokeres/Eze/Zubimendi remain unchanged. Two close-up replacement candidates failed Full-band clearance and a further Marmoush candidate failed frozen extraction, so those were rejected before import. Selected replacements preserve one fictional identity per Master and all three derivatives; exact provenance is retained.

Establish the permanent pre-import gate in Shared_Portrait_Art_Contract_v1.md section 30 and PortraitPrompts.md. SOURCE_READY requires Master QC plus real-production-recipe derivatives at actual gameplay size, same-team accepted composition references and Full overlay clearance. Known center drift, residue/halo, collar extraction, mannequin/sticker silhouette and flat-shirt/separation failures require source/metadata repair before import. No runtime art hacks, global recipe upgrade or acceptance by technical evidence. Future canonical promotions must also migrate any missing/fallback/rebind fixture to a real remaining non-canonical player.

Family remains v1.1; BalancedBust_v2 and QuietPitchBust_v2 remain current; Full CropOnly_v1 and deferred safe-zone remain frozen. Existing accepted 20 retain exact resources/metadata. Count stays 28/40; all Batch 3 roles remain PENDING USER PIE. Only nine changed-purpose Texture2D assets are reimported. Production C++, tests, gameplay, networking, data, numbers and serials are unchanged by this repair; no Editor build is required for art/metadata/docs-only changes.

## 2026-09-14 - Stage 8.4T source-space foreground candidate

Implement an isolated Player Card Family v1.2 CANDIDATE for exactly Reijnders, Marmoush, Dias, Donnarumma, Cherki, Ake, Saka and Rice. SourceSpaceForeground_v1 produces one crop-independent, source-bound foreground basis using existing pinned offline OpenCV facilities. Source-space anchors and bounded boundary reclassification replace crop-dependent crown membership and single-point component retention in the candidate. Hand/Pitch composition, framing, runtime dimensions/UV and all Masters/Full stay frozen. Broad probable-foreground trials that retained stadium lamps were rejected; they are not production outputs.

The production v1.1 generator, manifest/provenance, all assets and acceptance records are untouched. Candidate code/provenance and before/after evidence distinguish extraction version from unchanged BalancedBust_v2 / QuietPitchBust_v2 composition profiles. Candidate outputs remain under Saved/Stage8_4/FamilyPrototype; any UE preview uses transient textures and existing UMG widgets, not production imports. Existing accepted artifacts retain acceptance; new candidate Hand/Shared are PENDING USER PIE, and Batch 3 remains pending. No full 28-player migration, formal v1.2 acceptance, C++/gameplay change or build is authorized by this prototype. Exact proposed preflight wording is in Shared_Portrait_Art_Contract_v1.md section 32, pending candidate acceptance.

## 2026-09-14 - Stage 8.4V family composition candidate

USER PIE feedback after foreground repair identifies roster-level scale, vertical position, horizontal balance and torso exposure as the next shared problem. Prototype exactly ten players: Reijnders, Marmoush, Dias, Donnarumma, Cherki, Havertz, Gyokeres, Ake, Saka and Rice. Preserve SourceSpaceForeground_v1 and all Master/Full/production resources. Isolate composition through fixed-alpha BEFORE/AFTER comparisons; do not resume the individual Master/crop repair path or migrate all 28.

Introduce BalancedBust_v3_candidate and QuietPitchBust_v3_candidate as offline composition identities. A shared deterministic face/eye/crown fit constrains head size and position while preserving pose, hair, neck and shoulder differences. Bounded shoulder evidence protects torso room; whole-body bounds do not set zoom. Existing crop metadata is unchanged and supplies only a weak preference. Pitch is included based on the independent ten-player comparison, with its existing visual language, background direction and runtime UV retained. Exact algorithm/tolerances and proposed future preflight wording are in Shared_Portrait_Art_Contract_v1.md section 33.

Production family remains v1.1; candidate family v1.2 and all new Hand/Shared outputs remain PENDING USER PIE. Existing acceptance records and Batch 3 pending status are untouched. Saved/Stage8_4/CompositionPrototype contains candidate provenance, fixed bases, actual-size sheets and temporary UE preview. No C++, Widget layout, drag, Full, bio, number, gameplay/networking or production package change; no build or broad gameplay regression is required. USER PIE must judge the entire roster column and healthy protection samples before any subsequent canonical migration. Staging and commit remain manual.

## 2026-09-14 - Stage 8.4U migration preflight and shared extraction escalation

USER-APPROVED FOR PRODUCTION MIGRATION: SourceSpaceForeground_v1 plus the accepted v3 Hand/Pitch composition prototypes. Intended production names are BalancedBust_v3 and QuietPitchBust_v3; formal family v1.2 remains conditional on a successful 28-player migration. The direction approval does not waive source/foreground integrity.

All 28 candidate families generate with the unchanged accepted algorithms/crops; the ten-player prototype reproduces exactly (10 alpha / 20 output hashes). All 56 Hand/Shared candidates differ from production and meet numeric composition bounds. Visual source-versus-alpha preflight nevertheless fails on Saka, Saliba, Donnarumma, Foden, Rodri, Dias and Reijnders: source hair/temple/ear/jaw/neck pixels are incorrectly removed, with Donnarumma the clearest severe case. Four of these are already byte-identical to the prior ten-player prototype. Determinism and family direction acceptance do not prove source-contour integrity.

Result: NEEDS SMALL REPAIR; stop before production replacement or runtime import. No per-player exception, new crop or Master repair is introduced. Escalate repeated defects to shared foreground extraction review; retain the approved composition direction. Formalize the permanent source/alpha and family pre-import gate in Shared_Portrait_Art_Contract_v1.md section 30; section 34 and the existing Stage8_4_Generation.json hold this failed migration record.

Production remains family v1.1, canonical 28/40. All Master/Full and production Hand/Shared bytes, manifest/provenance/crops, original 20 accepted families and Batch 3 pending roles remain unchanged. Six prototype code/test/helper files are preserved, promotion/cleanup deferred, and no temporary wrapper is used by production. Focused foreground/composition tests 11/11 PASS plus targeted alpha reproduction/invalid-source rejection do not override the visual gate. No UE import, build, broad gameplay or final USER PIE request is performed for an un-applied migration. Staging/commit remain manual.


## 2026-09-15 - Stage 8.4W foreground integrity candidate, composition frozen

Repair extraction only for the seven Stage 8.4U problem Masters, with Ake/Rice/Eze as the exact protection set. Keep SourceSpaceForeground_v1 reproducible and introduce SourceSpaceForeground_v2_candidate in a separate offline module. Actual v1 stage captures locate initial-classification loss; Reijnders additionally loses valid ear components. Local head/neck appearance passes, contour-supported interior seeds, bounded side-component adjacency and source-edge background veto address the shared defect without player/team branches or crop/Widget changes. Broad background removal that damaged dark hair and trials that enlarged Eze background were rejected.

The selected ten-sample candidate materially repairs all seven defect regions and preserves the three protection silhouettes at card size. It remains imperfect at tiny edges and retains a narrow inherited v1 Eze temple strip; engineering review is not semantic pixel certification or USER PIE. Fixed v1 anchors/background and immutable v3 composition isolate the alpha change: twenty old candidate PNG hashes reproduce, and twenty new/old transforms match. Focused source-region, background, bounds, deterministic/version and invalid-source checks pass; evidence and transient review tooling stay under ignored Saved/Stage8_4/ForegroundRepair.

Status: PENDING FOREGROUND FAMILY USER PIE. Do not retry the 28-player production migration or import any production textures in this Stage. Production family remains v1.1, all Masters/Full and current Hand/Shared resources, crop metadata, membership, existing 20-player acceptance and Batch 3 pending roles remain unchanged. Section 30 now explicitly requires left/right temple inspection and rejects generation/metrics-only approval; section 35 documents the candidate and its limits. No C++ build or gameplay/network regression is required for this isolated Python/art-review change. Staging and commit remain manual.

## 2026-09-15 - Stage 8.4U-Retry formal family v1.2 migration

The user explicitly passed the family composition direction and the Stage 8.4W foreground V2 USER PIE and authorized production migration. Promote SourceSpaceForeground_v2, BalancedBust_v3 and QuietPitchBust_v3; candidate names are historical only. Preserve the accepted W input relationship: the deterministic v1 prior supplies anchors/background, and v2 supplies rendered foreground. Make these inputs explicit in production instead of using the review-only render patch. All ten W alpha hashes and twenty accepted sample PNGs reproduce.

Generate/review all 28 families before replacement. Engineering review finds no new severe visible outlier; accepted tiny edge imperfections remain nonblocking polish. Hand is 28 changed / 0 identical; Shared is 28 / 0. Replace only these 56 PNGs and import only the 56 changed Texture2Ds. Original 20 Hand/Pitch roles return to PENDING USER PIE; their unchanged Full roles stay accepted. Batch 3 retains all 24 pending roles. Canonical count stays 28/40. Master/Full bytes and Full metadata, crop guides, runtime UI/UV/drag, bio/number/serial and gameplay/networking remain unchanged.

Formalize an exact-selection, source/crop/output/code-bound engineering preflight receipt before production writes. Missing/stale/partial receipts fail closed; no automatic pixel metric grants visual acceptance. Source, alpha, actual-size family review and per-role byte lifecycle remain the long-term gate. Production uses the versioned core modules and one generator; historical wrappers and ignored caches are not production dependencies.

Family v1.2 technical migration requires final family USER PIE for visual closeout. This is not SAFE TO COMMIT; staging/commit remain manual. The exact attempt and review receipt are recorded in Stage8_4_Generation.json and ignored Saved/Stage8_4/FinalFamilyMigration.

## 2026-09-15 - Stage 8.4 final family USER PIE PASS and production closeout

The user explicitly accepts the final Player Card Family v1.2 and all eight Batch 3 Hand/Shared/Full families. Synchronize the current 28 canonical families to 84 USER PIE ACCEPTED roles; this supersedes U-Retry's pending visual gate without rewriting the failed U preflight or T/V/W/R history. Foreground SourceSpaceForeground_v2, Hand BalancedBust_v3, Pitch QuietPitchBust_v3 and Full CropOnly_v1 remain formal. Full continues directly from the same Master, without small-card alpha. No art bytes, crop guides, runtime packages, C++ behavior or UI layout change during this closeout; earlier approved Batch 3 authoring/repair remains in the accumulated delta. Final diff hygiene removes surplus EOF blank lines from the untracked Batch 3 Python/C++ tests; the C++ source touch triggers the requested incremental Editor build.

Promote the durable future-player gate in Shared_Portrait_Art_Contract_v1.md section 30 to v1.2. Require one Master, explicit formal profiles, sound athletic source anatomy/garment without fake text or baked number, independent left/right source-versus-alpha inspection, actual 96x64 Hand / 130x112 Pitch comparison with accepted team references, modest crop versus Master reauthoring, no player-specific runtime hacks, and family-level escalation for repeated defects. New/changed outputs stay pending until explicit approval; unchanged accepted roles stay accepted. Migrate genuine missing/fallback/rebind fixtures before promoting their player.

Retain the versioned foreground/composition modules and PlayerPortraitPreflight as production dependencies; retain their focused tests as development tools. Remove only the untracked PrototypePlayerPortraitForeground.py and PrototypePlayerPortraitComposition.py wrappers, after checking no production/test dependency. Their fixed-roster, ignored-cache-bound workflows are superseded by formal generator preview/preflight. Historical evidence remains ignored. Canonical count stays 28/40 (14 per team); JohnStones remains the real missing-art fixture. Full safe-zone stays DEFERRED. Staging and commit remain manual.

## 2026-09-15 - Stage 8.4 checkout-stable implementation provenance
