...
- 日期：2026-06-26
- 决策：每名玩家基础进攻次数为 3；初始牌组稀有度积分较高的一方额外获得 1 次；积分相同则双方都不获得该项加成；之后各自掷 D6，1-3 额外 1 次，4-6 额外 2 次。
- 决策：双方总进攻次数相同时，先比较初始牌组稀有度积分，积分高的一方后手；积分也相同则双方掷 D6，点数大的一方后手；平点继续掷直到不同。
