...
阶段 4.40 已建议将当前逻辑接口暂定为 External API v1，并暂停新增外部包装层。冻结范围、职责交叉检查和风险见 `CoreRules_ExternalApiSurfaceFreezeReview.md`。
...
阶段 4.42 新增 v1 生命周期回归测试，继续覆盖 SetupView、StateView、Preflight、ExternalTurnController、提交结果与提交后 StateView，并锁定非法请求原子性和提交链最终重验。阶段 4.42.1 修正终局 Guard 判断顺序：`CurrentAttacker=None` 且双方机会耗尽时属于合法终局；仍有机会时仍为未初始化 / 非就绪 / 不可提交。最终提交的 `ResultView.bMatchEnded` 与同一 AfterState 的 `ExternalStateView.bIsMatchFinished` 已保持一致。当前 CoreRules 为 453/453 测试通过。
...
- 最后一次提交完成后，优先从 `FMatchPlayExternalTurnControllerResult.ResultView.ExecutionSummary` 读取 `bMatchEnded` 和权威 `MatchResultType`，并从 ResultView 读取提交前后比分。
- 需要完整最终状态摘要时，对 `ControllerResult.AfterState` 再调用 `MatchPlayExternalStateViewQuery::BuildView`。
- 仅持有 `FMatchPlayState` 时，`MatchPlayExternalStateView` 可以读取 `bIsMatchFinished` 与最终比分，但当前不能返回权威胜者 / 平局枚举。
- 外部系统不应复制 `MatchResultResolver` 逻辑。需要权威结果时应保留最后一次 Controller 结果。

阶段 4.41 暂不建议新增 `MatchPlayExternalCompletionView`，继续维持 External API v1 冻结。只有确认存在“仅凭持久状态重建权威结果”的真实需求时，才应重新评审窄范围只读 Completion Query / View。完整审查见 `CoreRules_MatchCompletionReadModelReview.md`。
...
- 优先验证真实外部调用方并补足现有 v1 入口，不再预先增加 ExternalXXXView、Controller 或 Facade。
