...
本文档记录阶段 4.40 对 CoreRules 外部 API 的冻结审查。阶段 4.42 / 4.42.1 完成后，当前基线为 453/453 测试通过。
...
阶段 4.42 已用 `MatchPlayExternalApiV1LifecycleTests` 锁定上述生命周期，包括初始化后读取、状态级就绪、具体请求预检、合法提交、结果读取、提交后状态读取、非法请求原子性和提交时重验。阶段 4.42.1 修正终局 Guard 顺序后，`CurrentAttacker=None` 且双方机会耗尽被识别为合法终局；若仍有机会，则继续保持未初始化 / 非就绪 / 不可提交。ResultView 与同一 AfterState 的 StateView 结束语义保持一致。
...
- 阶段 4.42 / 4.42.1 只补充生命周期回归覆盖和既有 Guard 语义一致性，不扩展 External API v1 表面。
