...
本文记录阶段 4.54 对 `FSingleCardFormulaInputContract` 与 `FFormulaResolverInput` 之间转换边界的评审，并合并同步阶段 4.55 Assembler 实现和 4.56 独立验收结果。阶段 4.57 至 4.60 已继续完成 Resolver 执行边界、验收和链路完成度 Review。当前 CoreRules 为 521/521 测试通过；这些阶段仍属于第 4 部分，不进入第 5 阶段。
...
- 阶段 4.55 已新增独立、纯逻辑、无状态的 Resolver Input Assembly 模块。
- 模块名为 `FSingleCardFormulaResolverInputAssembler`，文件为 `SingleCardFormulaResolverInputAssembler.h/.cpp/Tests.cpp`。
...
- 阶段 4.55 只支持直接属性取值的单卡双边组装，不实现平均、取半、求和、技能修正或多卡组合。
- 阶段 4.56 独立验收通过，未发现越界调用或验收级测试缺口，不需要 4.56.1。
...
命名保留 `SingleCard` 和 `ResolverInput`，避免被误解为技能公式引擎或通用多卡组装器。
...
- `SingleCardFormulaResolverInputAssembler.h`
- `SingleCardFormulaResolverInputAssembler.cpp`
- `SingleCardFormulaResolverInputAssemblerTests.cpp`

新增：

- `FSingleCardFormulaResolverInputAssembler`
- `FSingleCardFormulaResolverInputAssemblyInput`
...
- `InvalidSide`。
- `InvalidField`。
- 双方下层 `FSingleCardFormulaInputAssemblyQueryResult`。
- 成功组装的 `FFormulaResolverInput`。
...
`ContextTag` 当前没有对应的 `FFormulaResolverInput` 字段。4.55 没有为它发明 Resolver 字段；它仅随下层 Query Result 保留用于诊断，没有被误写为 FormulaType 或 Match Phase。
...
这些测试证明 FormulaResolver 如何消费已组装输入，但不单独证明 Assembler 的字段映射正确。

阶段 4.55 没有改造 FormulaResolver 或重写现有 Resolver 测试，并新增了独立 Assembler 测试，覆盖：
...
Assembler 测试没有通过调用 FormulaResolver 来证明组装正确，而是直接断言 `FFormulaResolverInput` 的目标字段。可在后续独立集成阶段再评审端到端调用。
...
8. 结构化失败、下层 Query 错误保留和输入不变。
9. 12 个独立 Assembler 自动化测试和禁止依赖测试。

阶段 4.56 独立验收结论：

- 通过。
- 未发现 FormulaResolver、Snapshot Query、MatchPlay、External API v1、随机数或数据源层越界调用。
- 未修改 FormulaResolver 实现或测试。
- 未发现字段映射错误。
- 未发现验收级测试缺口。
- 不需要 4.56.1 修正阶段。
- CoreRules 全量测试为 514/514 通过。
- UE5 Development Editor 编译与 UnrealHeaderTool `-WarningsAsErrors` 复验通过。

非阻塞可选增强：

- 使用表驱动测试覆盖全部 16 项 `ESingleCardFormulaAttribute`。
- 在全部失败测试中统一断言 `ResolverInput` 保持默认不可用。

这些增强不影响 4.55 当前验收结论。

## 已落地执行边界

阶段 4.58 已在 Assembler 与 FormulaResolver 之间新增 `FSingleCardFormulaResolutionExecutor`：

- 只接收完整 `FFormulaResolverInput`。
- 不反向读取 Query Result、Snapshot Query 或调用 Assembler。
- 做当前单卡范围的最小防御校验后，只调用一次 FormulaResolver。
- 使用独立结构化 Result，保留输入副本与 Resolver 原始返回值。
- 不映射为比赛状态、进球流程或 External API 结果。

阶段 4.59 轻量验收通过；阶段 4.60 确认当前单卡公式解析链路能力完整，暂不新增 Pipeline。完整记录见 `CoreRules_FormulaResolutionExecutionBoundaryReview.md`。
...
当前 Assembler 仍不包含 FormulaResolver 调用、MatchPlay / External API v1、Determination、技能效果、多卡组合、随机数、TieBreaker、Provider、DataTable、卡牌数据库、Content、UI、蓝图、联网、Steam、完整比赛循环，也不引入抽牌、洗牌、手牌或牌库语义。
