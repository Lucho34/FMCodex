...
| `SingleCardFormulaInputContractValidator` | 只读验证单卡公式输入契约。 | `Validate` 检查 CardId、支持的 FormulaType、角色 / 属性组合、显式外部 D6、显式且有限的 Modifier 和非负 TurnIndex；结构化返回失败字段。 | 不调用 FormulaResolver，不实现 FormulaInputAssemblyQuery，不读取 Snapshot、Provider、DataTable 或 MatchPlay；当前不验证角色与实际 Snapshot GK 身份，也不定义 Modifier 数值范围。 | 否 | `SingleCardFormulaInputContract` |
