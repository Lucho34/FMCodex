...
纯 Catalog/Resolver 实现已经关闭，但 `FMatchPlayState` 尚未持有 Catalog，opening input / initializer 尚未验证和值拷贝 Catalog，也没有 match-long mutation protection。per-side Snapshot authority、ordinary writer、availability、Automatic Finish、GK writer、Resolution consumer、terminal projection 与 `CompleteCurrentAttack` 继续开放。下一唯一入口为 `7.85 MatchPlay Slot Catalog Ownership + Opening Initialization Binding Capability Selection + Minimum Contract Review`（GPT-5.6 Sol High）。
