...
在 7.78 的历史关闭快照中，普通 Deployment placement、Slot / Zone / Occupancy authority、Deployment Availability、Automatic Finish、永久 GK 使用事实、GK Deployment writer、temporary GK writer、Resolution consumer、terminal projection、`CompleteCurrentAttack`、Through Ball completion consumer、Formal Abort、Direct Shot、Shooter Snapshot 和旧 lower-level flow 迁移仍未实现。后续章节已关闭其中的 Slot/Catalog、Snapshot、ordinary deployment 与 GK deployment；其余 Deferred 范围及既有债务状态以文档末尾当前 Contract 为准。
...
## Neutral Physical Slot + Relative Tactical Zone Contract（7.79–7.84）

### Identity 与 implemented value types
...
提交 `8a32cf3c59592898ff1e147ebd14b8f9b046bc9e` 已实现以下最小类型：
...
安全默认值为：`SlotId = NAME_None`、`NeutralSide = None`、`Slots = empty`；两个 enum 的 `None` 均为安全零值。Definition / Catalog 不包含固定 Zone、PlayerSide、owner、occupant、current attacker、坐标、UI direction 或 CurrentAttack placements。
...
Validate Result 为 `bSuccess / ErrorCode / ErrorMessage`，Error 顺序为 `None / EmptyCatalog / EmptySlotId / DuplicateSlotId / InvalidNeutralSide`。FindSlot Result 为 `bSuccess / SlotId / SlotDefinition / ErrorCode / ErrorMessage`，Error 顺序为 `None / InvalidSlotId / InvalidCatalog / SlotNotFound`。Validator 不排序、去重、规范化、自动修复或修改 Catalog；FindSlot 返回 Definition 值拷贝，不暴露内部可修改引用或指针。
...
Result 精确包含 `bSuccess / SlotId / CurrentAttackingPlayer / EvaluatedPlayerSide / NeutralSide / RelativeZone / ErrorCode / ErrorMessage`。Error 顺序为 `None / InvalidSlotId / InvalidCurrentAttackingPlayer / InvalidEvaluatedPlayerSide / InvalidCatalog / SlotNotFound`。Resolver 验证顺序固定为：SlotId → current attacker → evaluated side → Catalog non-empty → all Catalog SlotIds non-empty → global uniqueness → supported NeutralSide → requested Slot lookup → mapping → success。
...
为修复新增文件改变默认 Unity packing 后暴露的既有测试同名符号冲突，7.83.1 还只修改 `ThroughBallFeetFormulaResolutionCompositionTests.cpp` 与 `ThroughBallBehindDefenseP1FormulaResolutionCompositionTests.cpp`：分别增加唯一 named namespace，完整覆盖 file-local 实现。实际 corrective diff 为 2 files、8 insertions、0 deletions；注册字符串、21 / 18 数量、输入、Expected、公式与 assertions 不变。

7.83 初次审查为 `BLOCKED`，原因是默认 Unreal Build Tool Unity Build 的 translation-unit collision，不是 Catalog/Resolver Contract 或行为失败。7.83.1 修正为 `PASS`；7.83.2 独立确认 Catalog Contract、八映射、默认 Build、UHT、same-TU proof、Catalog 28/28、Feet 21/21、P1 18/18 和 CoreRules 1601/1601，结论 `PASS` 且 Safe to Commit。
...
## MatchPlay Ordinary Player Deployment Contract（7.93–7.97）

上一节是 7.92 历史快照；本节是当前权威 Contract。

### Request and authoritative inputs

`FMatchPlayOrdinaryDeploymentRequest` 精确为 `AttackSequence + RequestingSide + CardId + SlotId`。AttackSequence 是 stale-request guard。Request 不得携带 Snapshot、RelativeZone、NeutralSide、PositionTypes、CardUsage、CurrentAttackingPlayer、finished flags 或 NextLegalSide；这些全部由 `BeforeState` 提供。

### Single legality authority and first error

唯一 ordinary legality authority 是 `FMatchPlayOrdinaryDeploymentLegalityEvaluator::Evaluate(const FMatchPlayState&, const FMatchPlayOrdinaryDeploymentRequest&)`。它必须保持只读，Availability 与 Writer 必须复用它，不得存在第二套 legality。

首错顺序固定为：

1. MatchPlay State initialized；
2. CurrentAttack exists；
3. authoritative AttackSequence valid；
4. request AttackSequence matches；
5. Phase is Deployment；
6. CurrentAttackingPlayer valid；
7. RequestingSide valid；
8. CurrentLegalDeploymentSide valid；
9. RequestingSide is current legal side；
10. both-finished Deployment corruption rejected；
11. RequestingSide has not finished；
12. CardId valid；
13. SlotId valid；
14. side-aware Snapshot lookup；
15. CardUsage authority/status；
16. same-side CardId not already placed；
17. goalkeeper rejected from ordinary deployment；
18. Catalog lookup；
19. global Slot occupancy；
20. Relative Zone resolution；
21. Position eligibility；
22. success。

### Identity, CardUsage and placement

稳定身份为 `PlayerSide + CardId`。双方可使用相同 CardId；同一方在同一 CurrentAttack 中不能重复部署相同 CardId。Snapshot 严格查询 RequestingSide，不跨边 fallback。

普通部署成功不消费 CardUsage：卡牌继续位于 Available，不进入 Used。当前攻击防重由 `DeploymentPlacements` 负责；未来 `CompleteCurrentAttack` 才处理真实消费。

`FMatchPlayDeploymentPlacement` 只保存 `PlayerSide + CardId + SlotId`，不保存 Snapshot、Zone、NeutralSide、PositionTypes 或 attacker/defender role。本 Milestone 不新增 `FMatchPlayState` schema 字段。

### Slot, Zone and position

`FMatchPlayState::DeploymentSlotCatalog` 是 match-long Slot authority，`CurrentAttack.DeploymentPlacements` 是 occupancy authority。同一全局 SlotId 只能由任一方占用一次；不使用 per-side occupancy、ordinary-only cache 或 `SlotOccupants` map。

Relative Zone 必须使用 State-owned Catalog、SlotId、CurrentAttackingPlayer 和 evaluated RequestingSide 动态解析；UI 镜像、摄像机与屏幕方向不是规则输入。
...
多 PositionTypes 使用 OR：至少一项合法即可。GK 必须先返回 `GoalkeeperNotAllowed`，不得依靠模糊的 “not Attack / not Defense” 或把 GK 塞入普通矩阵。

### Availability

`FMatchPlayOrdinaryDeploymentAvailability::Query` 按 Catalog 原顺序枚举并对实际候选复用同一 Evaluator；LegalSlotIds 保持该顺序，不按 UI 方向排序。它不修改 State、不 Automatic Finish，也不接受 caller-supplied Snapshot、Catalog 或 Zone。

`bQuerySucceeded` 只表示 Availability 机制成功执行。wrong legal side、finished side、stale sequence、GK、unavailable/used card、全 occupied 或全 Position invalid 都可以成功查询但零合法 Slot；只有 Catalog 无法安全枚举才是 query failure。

### Atomic writer

`FMatchPlayOrdinaryDeploymentWriter` 的唯一公开入口为 `Deploy`，每请求恰好调用一次 Evaluator。成功流程为 Evaluate → Turn Rotation → copy State → append placement → apply NextLegalSide → atomic return。成功的实际状态差异只允许 `DeploymentPlacements` 和 `CurrentLegalDeploymentSide`；Phase 保持 Deployment，Runtime、CardUsage、Catalog、Snapshot authority、AttackSequence、ActionPoint、finished flags、GK activation、score 与 attack opportunities 均不变。

失败返回完整原 State。没有第二 public writer、legality bypass、Automatic Finish、Resolution 或 Completion。

### Shared turn rotation and Finish

`FMatchPlayDeploymentTurnRotation` 只接收 CurrentAttackingPlayer、ActingSide、attacker finished 与 defender finished，返回 NextPhase、NextLegalDeploymentSide 与精确 error。它不接收 State、不处理 CardUsage、ordinary/GK legality 或 Resolution consumer。

- 对方未 Finish：Deployment → Opponent(ActingSide)；
- 对方已 Finish：Deployment → ActingSide；
- 双方均 Finish：Resolution → None；
- errors：`InvalidCurrentAttackingPlayer / InvalidActingSide`。

finished flags 是相对于 current attacker/defender 的角色事实，不是固定 PlayerA/PlayerB。`FMatchPlayFinishDeployment` 复用同一 Rotation：第一方 Finish 后仍在 Deployment，第二方 Finish 后进入 Resolution；允许空 deployment Finish并保留 CurrentAttack、placements、Catalog、Snapshot authority 与 CardUsage。Finish 不完成 CurrentAttack，也不执行 Resolution consumer。

### Unity correction and acceptance

7.96 只因 clean-tree Unity `C2668 IsPlayer` name lookup ambiguity 未通过。7.96.1 提交 `0317a67` 从 Rotation implementation 删除 namespace-wide using，并完整限定 `MatchPlayDeploymentTurnRotationImplementation::IsPlayer / OtherSide`，不改变运行时 Contract。7.96.2 真实 `Module.FMCodex.6.cpp` 同时包含 Begin、Rotation、RotationTests 并编译通过；clean-tree Unity Rebuild、UHT、link PASS，generated files 0、adaptive exclusions 0、collision None。

最终独立基线为 Legality 30/30、Availability 10/10、TurnRotation 8/8、Writer 18/18、Ordinary 66/66、Begin 17/17、Finish 23/23、Catalog 28/28、Snapshot Authority 18/18、State 9/9、MatchPlay 490/490、CoreRules 1712/1712。

## MatchPlay Goalkeeper Deployment Contract（7.98–7.103）

上一节末尾的 future GK 条款是 7.97 历史入口。本节是当前已实现、已独立验证的权威 Contract。

### Persistent usage, transient activation and formula boundary

`FMatchPlayState::GoalkeeperUsageState` 是 match-long、per-side authority，类型为 `FMatchPlayGoalkeeperUsageState`。新比赛双方为 false；Begin、Finish、AttackFlow 与攻守互换均保留，只有创建新比赛 State 才重置。唯一 mutation authority 是纯值 `FMatchPlayGoalkeeperUsageStateResolver::MarkUsed`。

`FMatchPlayCurrentAttackState::bCurrentDefenseGoalkeeperActivated` 只属于当前攻击，Begin 新攻击重置 false，成功 GK writer 设为 true。Persistent Used、Current Attack Activation、Formula Participation 是三个独立概念。legacy `FPlayerMatchState::bUsedGoalkeeperActivation` 为 non-authoritative；不得把 CardUsage、placement 或 `FPlayerRuntimeState` 当作 match-long GK authority。本 Milestone 不修改任何 Formula。

### Request and single legality authority

`FMatchPlayGoalkeeperDeploymentRequest` 精确为 `AttackSequence + RequestingSide + CardId + SlotId`；其余所有事实从 `BeforeState` 推导。唯一 legality authority 是 `FMatchPlayGoalkeeperDeploymentLegalityEvaluator::Evaluate`，保持只读；Availability 复用它，Writer 每请求只调用一次。它不调用 MarkUsed、不写 activation/placement/rotation/CardUsage，也不调用 ordinary Position eligibility。ordinary evaluator 继续以 `GoalkeeperNotAllowed` 拒绝 GK。
...
10. both-finished Deployment corruption rejected；
11. RequestingSide has not finished；
12. RequestingSide is current defender；
13. CardId valid；
14. SlotId valid；
15. side-aware Snapshot lookup；
16. Snapshot is goalkeeper；
17. CardUsage valid and Available；
18. GoalkeeperUsageState query；
19. persistent/activation/current placement consistency；
20. current attack not already activated；
21. side has not already used GK this match；
22. Catalog and Slot lookup；
23. global Slot occupancy；
24. Relative Zone resolution；
25. resolved zone is defender Backfield；
26. success。

不相关 underlying error 必须保持 None/default；所有 evaluator 路径保持 State 不变。

### Defender, identity, CardUsage and consistency

防守方由 `RuntimeState.CurrentAttackingPlayer` 动态取另一方，不能固定为 PlayerA/PlayerB。只有当前防守方且轮到其合法部署时可以使用。Snapshot identity 为 `RequestingSide + CardId`，不跨侧 fallback，必须是该方真实 GK。

GK 必须仍在 CardUsage Available。成功不加入 Used、不 discard、不迁移卡牌区域；once-per-match 由 dedicated usage authority 阻止。consistent `persistent=true / activation=true / matching count=1` 优先返回 `GoalkeeperAlreadyActivatedThisAttack`；跨攻击 `true / false / 0` 返回 `GoalkeeperAlreadyUsedThisMatch`。`false / true / 任意`、activation 与 count 不一致、或未 activation 但 count>0 都是 `InvalidGoalkeeperUsageState`。匹配身份为 `PlayerSide + CardId`。

### Shared Slot, Backfield and placement

GK 复用 `FMatchPlayDeploymentPlacement(PlayerSide + CardId + SlotId)`；`CurrentAttack.DeploymentPlacements` 是 ordinary/GK 唯一共享 occupancy authority。任一 placement 占用全局 SlotId 即阻止双方和两类 deployment 再占用；不使用 per-side occupancy、GK-only cache 或第二 authority。

Slot 必须存在于 State-owned Catalog 且未占用。Relative Zone 只由 Catalog、SlotId、CurrentAttackingPlayer、RequestingSide 解析，必须等于 defender Backfield；Zone 不持久化。GK 绕过 ordinary PositionTypes 矩阵。

### Availability

`FMatchPlayGoalkeeperDeploymentAvailability::Query` 只枚举 State-owned Catalog，保持 Catalog 与 LegalSlotIds 原顺序，每个候选复用同一 evaluator 并保留完整 SlotResults。合法 Catalog + 零合法 Slot 是成功查询；Catalog 不可安全枚举才失败。Availability 不修改 State、不打出 GK、不 MarkUsed、不 activation、不 append placement、不 rotation、不 Automatic Finish。

### Atomic writer and rotation

`FMatchPlayGoalkeeperDeploymentWriter::Deploy` 是唯一 public writer。流程固定为 Evaluate exactly once → shared Turn Rotation → MarkUsed → copy State → append GK placement → apply updated usage → activation=true → apply legal side → atomic success。private finalizer 不是绕过入口。

成功只改变 `DeploymentPlacements`、请求方 usage bool、`bCurrentDefenseGoalkeeperActivated`、`CurrentLegalDeploymentSide`。Runtime、CardUsage、Catalog、Snapshot authority、phase、sequence、ActionPoint、finished flags、attacker、score、attack counts/opportunities 和另一方 usage 均不变；Phase 保持 Deployment。

Legality、rotation、MarkUsed 或任何请求失败均返回完整 BeforeState。对方未 Finish 时轮到对方；对方已 Finish 时 legal side 保持请求方；防守方已 Finish 返回 `RequestingSideAlreadyFinished`；双方已 Finish 不进入成功 writer。Writer 不执行 Automatic Finish、Resolution 或 Completion。

### Cross-attack lifecycle, verification and deferred scope

当前攻击首次成功产生 `persistent=true / activation=true / count=1`；当前攻击再次请求是 AlreadyActivated。下一攻击为 `true / false / 0`；再次请求是 AlreadyUsed，不是 corruption。
