...
### Frozen future GK behavior — not current implementation

`GK Deployment Implemented: NO`。未来必须：

- 只允许由 CurrentAttackingPlayer 动态推导出的当前防守方，在轮到该方合法部署时主动使用；
- 查询该方真实唯一 GK，并限制每方整场成功一次；
- 指定非空、Catalog-valid、未被任一方占用的共享 SlotId；
- 使用 CurrentAttackingPlayer + defending RequestingSide + SlotId 解析且要求 Defender Backfield；
- 绕过普通 PositionTypes 矩阵；
- 成功建立 match-long used fact 与 current-attack transient activation，同时 GK 卡仍保持 Available；共享 Slot occupancy 记录不构成普通 CardUsage 牌区迁移；
- 使用共享 rotation，失败保持全部状态原子不变。

冻结的是统一 global occupancy authority，不是存储形态。7.98 必须在“追加现有 placement”和“专用 GK placement 字段 + 统一 occupancy query”之间审查；当前不得宣称任一方案已决定，也不得建立两套不一致 occupancy。

Automatic Finish、Resolution consumer、terminal projection、CompleteCurrentAttack、Formal Abort、Direct Shot、Shooter Snapshot migration、lower-level flow migration、External gameplay API 与 UI/Blueprint integration 仍未实现。
