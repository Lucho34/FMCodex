...
## Stage 8.3C.1 — Global biography clearance (USER PIE ACCEPTED)

This narrow amendment supersedes the C.5 biography width and legacy primary-date reset only. Every Full card, including canonical, legacy, missing-art and future unrouted identities, uses a 90-unit biography panel and four-unit horizontal padding: an 82-unit fact lane. Labels remain nine-point and all facts use the accepted canonical 12-point size; legacy primary dates now inherit that same size. Legacy fact rows retain their vertical padding but no longer add a second horizontal inset. All four ordered rows and `—` missing-value behavior remain.

Narrow from the left. Preserve the existing skin-specific top/right anchors: canonical normal 18/10 inside the hero, canonical three-skill 12/10, legacy 10/5. Canonical left clearance increases six units with its right frame clearance unchanged; legacy increases ten units. All vertical row/separator/capacity rules remain. There is no portrait shift, UV change, per-player offset, art generation or resource change. Frame, rating/caption, name/number, attributes, skills, chapter rules, footer and 360×540 size retain Family v1.1 contracts.

Hover completion is independent of new-drag availability. Synchronous deployment refresh can replace the named Hand node before UMG delivers its Operation completion. Every rendered Rack card forwards completion even if it cannot start a new drag, allowing the shared screen to release Hand/Pitch hover suppression. Start-drag legality remains driven by the existing presentation choice. The focused native-event regression exercises canonical legal deployments through 8/9/10, including a final source-node replacement with no legal destinations; real Slate pointer evidence is separate from USER PIE acceptance.

Acceptance closeout: the user explicitly confirms Stage 8.3C and Stage 8.3C.1 USER PIE PASS. The final-deployment Hand/Pitch hover repair and global biography clearance are accepted, with no further blocking repair required. Canonical bio width changed 96 to 90 and legacy width 100 to 90; horizontal padding remains four, usable text width 82, values 12-point and labels nine-point. No per-player offsets are introduced. The seven Batch-2 migrations, including Timber crop metadata, are accepted; canonical coverage remains 18, with Cherki and Ake SOURCE_MISSING. Family v1.1 remains accepted/frozen, and Batch 3 has not started. Bernardo residual face/bio proximity is a non-blocking P2 future family-polish item. Earlier pending checkpoints remain historical.
