...
Near / Long / Penalty enter only after the Legacy Set Piece Type D6 reveal. Their selection layout
...
inspector is cleared on leaving selection. Methods use peer titles and the existing diagram family; Penalty distinguishes a direct spot shot from a central lofted chip.
Direct contests, including normal Penalty, use the shared two-sided composition. Panenka uses one visible die and no opposing Formula. Early miss and dual-die branches
