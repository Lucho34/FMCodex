"""Import Stage 6.13.1.3.10.4.2 D3 Runtime192 candidates through UE."""
...
SOURCE_ROOT = ROOT / "ArtSource" / "UI" / "Diagnostics" / "HandMicroReferenceA" / "Runtime192"
DESTINATION = "/Game/Developers/FMCodex/HandMicroReferenceA"
ASSET_NAMES = (
    ...
    "T_Prototype_ManchesterCity_GianluigiDonnarumma_HandMicro_ReferenceARuntime192",
    "T_Prototype_ManchesterCity_ErlingHaaland_HandMicro_ReferenceARuntime192",
)
...
for asset_name in ASSET_NAMES:
    ...
    if not source.is_file():
        raise RuntimeError(f"Missing D3 Runtime192 source: {source}")
    ...
    if not unreal.EditorAssetLibrary.save_loaded_asset(asset, only_if_is_dirty=False):
        raise RuntimeError(f"Failed to save D3 Texture2D: {asset_path}")
    unreal.log(
        "FMCODEX_HAND_MICRO_REFERENCE_A_IMPORT_ITEM=PASS "
...
unreal.log(
    "FMCODEX_HAND_MICRO_REFERENCE_A_IMPORT=PASS "
