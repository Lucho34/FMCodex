...
$importScript = Join-Path $PSScriptRoot "ImportPilotUIAssets.py"
$validateScript = Join-Path $PSScriptRoot "ValidatePilotUIAssets.py"
$common = @(
    $projectFile,
...
Invoke-UnrealPython -ScriptPath $importScript `
    -RequiredSentinel "FMCODEX_PILOT_IMPORT=PASS" `
    -Operation "Pilot UI texture import"

# A second Unreal process is intentional: importer success is not accepted as
# package-load proof. The validator independently discovers and loads both assets.
Invoke-UnrealPython -ScriptPath $validateScript `
    -RequiredSentinel "FMCODEX_PILOT_VALIDATION=PASS" `
    -Operation "Fresh-process pilot UI asset validation"
