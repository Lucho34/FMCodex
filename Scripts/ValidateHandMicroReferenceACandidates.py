"""Fresh-process validation for Stage 6.13.1.3.10.4.2 D3 assets."""
...
for asset_name in ASSET_NAMES:
    ...
    if not isinstance(asset, unreal.Texture2D):
        raise RuntimeError(f"Missing D3 Texture2D: {asset_path}")
    checks = {
    ...
    if failed:
        raise RuntimeError(f"D3 validation failed: {asset_path}: {failed}")
    unreal.log(
        "FMCODEX_HAND_MICRO_REFERENCE_A_VALIDATE_ITEM=PASS "
        f"asset={asset_path} dimensions=192x128 uv=full runtime_transform=none"
    )

unreal.log(
    "FMCODEX_HAND_MICRO_REFERENCE_A_VALIDATE=PASS "
