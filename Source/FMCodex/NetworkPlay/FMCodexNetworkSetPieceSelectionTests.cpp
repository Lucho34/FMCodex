...
bool FFMCodexSetPieceMethods::RunTest(const FString& P)
{
 ...
 for(auto* PC:{F.A,F.B})
 {
  ...
  TestEqual(TEXT("Exact offered resolution step"),V.EntryWait,D6==5?(Alt?Wait::NearAngledRoll:Wait::NearDirectAttackRoll):D6<=4?(Alt?Wait::LongFreeKickPowerRoll:Wait::LongFreeKickDirectAttackRoll):(Alt?Wait::PenaltyPanenkaRoll:Wait::PenaltyDirectAttackRoll));
  TestTrue(TEXT("Carrier set piece reuses shared resolution surface"),M.InlineFormula.bVisible); TestEqual(TEXT("Decisive action owner only"),M.Interaction.bCanContinue,PC==F.Attacker());
